...
fn test_cfg_basic_block_ten_line_cap_overflow() {
	// Construct a top-level block of 12 sequential let directives; the
	// basic-block caps at `max_block_lines` (10 in v0.8.0, raised from 6)
	// and collapses the remainder into a `(+K more)` overflow row.
	// The parser doesn't admit bare directive sequences at top level
	// without a multi-statement `.block` body; we rely on the implicit
	// multi-statement parser (`exprs.len > 1`) to produce the block wrapper.
	src := "[?let [= \$a 1] 1]
[?let [= \$a 2] 1]
[?let [= \$a 3] 1]
[?let [= \$a 4] 1]
