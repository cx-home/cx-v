...
// ── source: vcx/tests/identity_umbrella_test.v ──
...
// ── source: vcx/tests/identity_tier1_order_semantics_test.v ──
...
fn test_tier1_sequence_order_is_significant() {
	...
	assert h_tier1('[r (2, 1)]') != h_tier1('[r (1, 2)]'), 'sequence order must change the Tier-1 hash'
}

fn test_tier1_array_order_is_significant() {
	assert h_tier1('[r::int [3, 1, 2]]') != h_tier1('[r::int [1, 2, 3]]'), 'array order must change the Tier-1 hash'
}
...
fn test_tier1_attribute_order_is_significant() {
	...
	assert h_tier1('[rec a=1 b=2]') != h_tier1('[rec b=2 a=1]'), 'attribute order must change the Tier-1 hash'
}
...
fn test_tier1_map_key_order_collides() {
	...
	assert h_tier1('[rec {b: 2, a: 1}]') == h_tier1('[rec {a: 1, b: 2}]'), 'map key order must NOT change the Tier-1 hash'
}
...
[rec a=1]'
	...
	assert h_tier1(with_comment) == h_tier1(without_comment), 'comments must NOT change the Tier-1 hash'
}
...
fn test_tier1_distinct_pairs_are_well_formed() {
	...
	for p in pairs {
		assert h_tier1(p[0]).len == 73, 'expected tagged sha2-256 (73 chars) for `${p[0]}`'
		assert h_tier1(p[1]).len == 73, 'expected tagged sha2-256 (73 chars) for `${p[1]}`'
	}
}

// ── source: vcx/tests/feature_mask_agreement_test.v ──
