	...
	flush_section := fn [mut cur, mut section, mut lines_buf]() {
		if ct := cur {
			if sec := section {
				...
				cur = ?Test(Test{ ...ct, sections: {sec: lines_buf.join('\n')} })
				// merge sections properly:
				if mut t := cur {
					t.sections[sec] = lines_buf.join('\n')
					cur = t
				}
			}
		}
...
fn run_test(t Test) []string {
	...
	input_cx, is_xml := if 'in_cx' in t.sections {
		t.sections['in_cx'] or { '' }, false
	} else if 'in_xml' in t.sections {
		t.sections['in_xml'] or { '' }, true
	} else {
		return failures
	}
	...
	// Parse
	result := if is_xml {
		cx.parse_xml_cx(input_cx) or {
			failures << 'parse error: ${err}'
			return failures
		}
	} else {
		cx.parse_cx(input_cx) or {
			failures << 'parse error: ${err}'
			return failures
		}
	}
