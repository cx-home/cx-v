...
// ryu_f64.v — CX-OWNED Ryū float formatting (I1 identity epoch, stream 12,
// owner-ruled L18 / wart W-14-float; W-3 rides the same commit).
//
// canonical.md §2.5 makes the float surface part of Tier-1 identity:
// shortest round-trip digits, mandatory decimal point, scientific only when
// SHORTER than fixed (tie → fixed), lowercase `e`, no `+` / no zero-padding
// in the exponent, mantissa always carrying `.` (`1.0e10`), `-0.0` distinct.
// The emitter previously rode V's `f64.str()` — so a V-upgrade formatting
// tweak could silently re-hash every stored float. This file freezes the
// digit generation INSIDE Ring 0.
//
// Provenance: the shortest-representation core (cx_ryu_f64_to_decimal and
// the two power-of-five tables) is vendored from the V standard library's
// strconv (vlib/strconv/{f64_str.c.v, utilities.v, tables.v} at the pinned
// fork commit; MIT, Copyright (c) 2019-2024 Dario Deledda), itself an
// implementation of Ryū (Ulf Adams, PLDI 2018, doi:10.1145/3192366.3192369)
// following github.com/cespare/ryu. Vendored 2026-08-05 for I1. FROZEN:
// byte-for-byte output stability is a spec contract — never re-sync this
// file from vlib without an owner-ruled identity migration.
//
// The only intentionally-shared dependencies are fixed-semantics intrinsics
// (math.bits.mul_64 / trailing_zeros_64 and the f64 bit pattern), which no
// library upgrade can change without changing what a u64 multiply means.

const cx_ryu_mantbits64 = u32(52)
const cx_ryu_bias64 = 1023
const cx_ryu_pow5_num_bits_64 = 121
const cx_ryu_pow5_inv_num_bits_64 = 122
...
@[inline]
fn cx_ryu_bool_to_u32(b bool) u32 {
	if b {
		return u32(1)
	}
	return u32(0)
...
@[inline]
fn cx_ryu_bool_to_u64(b bool) u64 {
	if b {
		return u64(1)
	}
	return u64(0)
...
// floor(log_10(2^e)); valid for 0 <= e <= 1650.
@[inline]
fn cx_ryu_log10_pow2(e int) u32 {
	return (u32(e) * 78913) >> 18
}

// floor(log_10(5^e)); valid for 0 <= e <= 2620.
@[inline]
fn cx_ryu_log10_pow5(e int) u32 {
	return (u32(e) * 732923) >> 20
}

// ceil(log_2(5^e)), or 1 if e == 0; valid for 0 <= e <= 3528.
@[inline]
fn cx_ryu_pow5_bits(e int) int {
	return int(((u32(e) * 1217359) >> 19) + 1)
}

@[inline]
fn cx_ryu_shift_right_128(v CxRyuUint128, shift int) u64 {
	// In Ryū's 64-bit path the shift is always in [2, 59].
	return (v.hi << u64(64 - shift)) | (v.lo >> u32(shift))
}

fn cx_ryu_mul_shift_64(m u64, mul CxRyuUint128, shift int) u64 {
	hihi, hilo := bits.mul_64(m, mul.hi)
	lohi, _ := bits.mul_64(m, mul.lo)
	mut sum := CxRyuUint128{
		lo: lohi + hilo
		hi: hihi
	}
	if sum.lo < lohi {
	...
	return cx_ryu_shift_right_128(sum, shift - 64)
}

fn cx_ryu_pow5_factor_64(v_i u64) u32 {
	mut v := v_i
	for n := u32(0); true; n++ {
		q := v / 5
		r := v % 5
...
@[inline]
fn cx_ryu_multiple_of_power_of_five_64(v u64, p u32) bool {
	return cx_ryu_pow5_factor_64(v) >= p
}

@[inline]
fn cx_ryu_multiple_of_power_of_two_64(v u64, p u32) bool {
	return u32(bits.trailing_zeros_64(v)) >= p
}

// cx_ryu_f64_to_decimal_exact_int is the exact-integer fast path: a double
// whose value is a small integer converts by shifting, with trailing
// decimal zeros folded into the exponent.
@[ignore_overflow]
fn cx_ryu_f64_to_decimal_exact_int(i_mant u64, exp u64) (CxRyuDec64, bool) {
	mut d := CxRyuDec64{}
	e := exp - u64(cx_ryu_bias64)
	if e > u64(cx_ryu_mantbits64) {
		return d, false
	}
	shift := u64(cx_ryu_mantbits64) - e
...
// cx_ryu_f64_to_decimal converts a finite non-zero double (raw mantissa +
// biased exponent) to its SHORTEST decimal representation d.m × 10^d.e.
fn cx_ryu_f64_to_decimal(mant u64, exp u64) CxRyuDec64 {
	mut e2 := 0
	mut m2 := u64(0)
	if exp == 0 {
		e2 = 1 - cx_ryu_bias64 - int(cx_ryu_mantbits64) - 2
		m2 = mant
	} else {
		e2 = int(exp) - cx_ryu_bias64 - int(cx_ryu_mantbits64) - 2
		m2 = (u64(1) << cx_ryu_mantbits64) | mant
	}
	even := (m2 & 1) == 0
	accept_bounds := even
	...
	mv := u64(4 * m2)
	mm_shift := cx_ryu_bool_to_u64(mant != 0 || exp <= 1)
	...
	if e2 >= 0 {
		q := cx_ryu_log10_pow2(e2) - cx_ryu_bool_to_u32(e2 > 3)
		e10 = int(q)
		k := cx_ryu_pow5_inv_num_bits_64 + cx_ryu_pow5_bits(int(q)) - 1
		i := -e2 + int(q) + k
		mul := CxRyuUint128{
			lo: cx_ryu_pow5_inv_split_64[q * 2]
			hi: cx_ryu_pow5_inv_split_64[q * 2 + 1]
		}
		vr = cx_ryu_mul_shift_64(u64(4) * m2, mul, i)
		vp = cx_ryu_mul_shift_64(u64(4) * m2 + u64(2), mul, i)
		vm = cx_ryu_mul_shift_64(u64(4) * m2 - u64(1) - mm_shift, mul, i)
		if q <= 21 {
			// Only one of mp, mv, and mm can be a multiple of 5, if any.
			if mv % 5 == 0 {
				vr_is_trailing_zeros = cx_ryu_multiple_of_power_of_five_64(mv, q)
			} else if accept_bounds {
				vm_is_trailing_zeros = cx_ryu_multiple_of_power_of_five_64(mv - 1 - mm_shift, q)
			} else if cx_ryu_multiple_of_power_of_five_64(mv + 2, q) {
				vp--
			}
		}
	} else {
		q := cx_ryu_log10_pow5(-e2) - cx_ryu_bool_to_u32(-e2 > 1)
		e10 = int(q) + e2
		i := -e2 - int(q)
		k := cx_ryu_pow5_bits(i) - cx_ryu_pow5_num_bits_64
		j := int(q) - k
		mul := CxRyuUint128{
			lo: cx_ryu_pow5_split_64[i * 2]
			hi: cx_ryu_pow5_split_64[i * 2 + 1]
		}
		vr = cx_ryu_mul_shift_64(u64(4) * m2, mul, j)
		vp = cx_ryu_mul_shift_64(u64(4) * m2 + u64(2), mul, j)
		vm = cx_ryu_mul_shift_64(u64(4) * m2 - u64(1) - mm_shift, mul, j)
		if q <= 1 {
			...
			vr_is_trailing_zeros = cx_ryu_multiple_of_power_of_two_64(mv, q - 1)
		}
	}
	...
	if vm_is_trailing_zeros || vr_is_trailing_zeros {
		...
		out = vr + cx_ryu_bool_to_u64(vr == vm || round_up)
	}
...
// cx_f64_is_finite reports whether v is neither NaN nor ±Inf. Non-finite
// floats have NO canonical form (§1.3 / W-3) — the canonical lane refuses
// them; only display rendering ever spells them out.
pub fn cx_f64_is_finite(v f64) bool {
	return (cx_f64_bits(v) >> 52) & 0x7ff != 0x7ff
}

@[inline]
fn cx_f64_bits(v f64) u64 {
	return unsafe { *(&u64(&v)) }
}

// cx_format_float renders a double per canonical.md §2.5. Finite values get
// the CX-owned Ryū shortest form; NaN / ±Inf get DISPLAY-ONLY spellings
// (the canonical lane rejects them upstream — cx_reject_nonfinite).
pub fn cx_format_float(v f64) string {
	b := cx_f64_bits(v)
	neg := (b >> 63) != 0
	biased_exp := (b >> 52) & 0x7ff
	raw_mant := b & u64(0x000f_ffff_ffff_ffff)
	if biased_exp == 0x7ff {
		if raw_mant != 0 {
			return 'nan'
		}
		return if neg { '-inf.0' } else { '+inf.0' }
	}
	if (b << 1) == 0 {
		// Exponent-always (2b): a fixed-point `0.0` would re-type DECIMAL.
		return if neg { '-0.0e0' } else { '0.0e0' }
	}
	mut d, ok := cx_ryu_f64_to_decimal_exact_int(raw_mant, biased_exp)
	if !ok {
		d = cx_ryu_f64_to_decimal(raw_mant, biased_exp)
	}
	digits := d.m.str()
	l := digits.len
	point := d.e + l // value = 0.digits × 10^point

	// §2.5 AS AMENDED at I1 (owner ruling 2b): canonical floats ALWAYS
	// carry the exponent — fixed-point text is the DECIMAL kind's spelling,
	// so the two kinds are lexically self-describing (one spelling, one
	// kind, one address). Mantissa always carries `.`; lowercase `e`; the
	// exponent prints bare (no `+`, no zero-padding).
	rest := if l > 1 { digits[1..] } else { '0' }
	sci := digits[..1] + '.' + rest + 'e' + (point - 1).str()
	return if neg { '-' + sci } else { sci }
}
const cx_ryu_pow5_split_64 = [
...
const cx_ryu_pow5_inv_split_64 = [
