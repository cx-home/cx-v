...
fn run_fabric_serve(args []string) {
	...
	cfg := platform.parse_fabric_service_config(src) or {
	...
	mut srv := platform.new_fabric_server(cfg) or {
		eprintln('cx fabric-serve: ${err}')
		exit(1)
	}
	...
	if platform.svc_is_err(server) {
		eprintln('cx fabric-serve: bind ${bind_url}: ${platform.svc_err_text(server)}')
		exit(1)
	}
	platform.svc_install_shutdown_signals(server, cfg.drain_ms)
	if stdin_tether {
		platform.svc_install_stdin_tether('cx fabric-serve')
	}
	...
	if cfg.health != '' {
		health := platform.svc_listen('tcp://${cfg.health}')
		if platform.svc_is_err(health) {
			eprintln('cx fabric-serve: bind health ${cfg.health}: ${platform.svc_err_text(health)}')
			exit(1)
		}
		platform.svc_register_grpc_listener(health)
		spawn platform.fabric_serve_health_loop(mut srv, health)
	}

	spawn platform.fabric_liveness_sweeper(mut srv)
	...
	spawn platform.fabric_retention_sweeper(mut srv)
	...
	for m in cfg.mounts {
		...
		names << '${m.name} (tenant ${m.tenant}, ${platform.store_url_redact_userinfo(m.store_url)})'
	}
	...
	platform.svc_sd_notify_ready()
	platform.svc_sd_watchdog_start()

	platform.fabric_serve_accept_loop(mut srv, server, platform.svc_shutdown_requested)
	platform.fabric_serve_shutdown(mut srv)
