...
fn run_store_rotate_kek(args []string) {
	...
	code.caps_set_list(caps) or {
		eprintln('cx store-rotate-kek: ${err.msg()}')
		exit(2)
	}
	r := platform.svc_rotate_kek(url, old_id, new_id)
	if platform.svc_is_err(r) {
		eprintln('cx store-rotate-kek: ${platform.svc_err_text(r)}')
		exit(1)
	}
