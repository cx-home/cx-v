...
mut:
	...
	fd     int = -1
	closed bool
	// fd-streaming RSS (#52): the per-group framing buffers are transient
	// (built, written, dropped). This writer used to force a gc_collect()
	// every 2 MiB of payload because vgc's old live-set pacer (256 MB fixed
	// floor) never reclaimed them across a long write loop. The collector now
	// paces itself (#71 adaptive headroom, third_party/v vgc): a churn loop
	// with a small live set collects every few MB by construction, so the
	// manual drive is gone. Guards: the cmp-005 1.5× RSS bound and the
	// pacer-only cases in vgc_large_span_reclaim_test.v.
}
