...
fn grpc_client_call(rb &RemoteBackend, method string, msg []u8) (GrpcClientResult, cx.Node, bool) {
	...
	// S3 (G1a): sign each call with the client's XSP identity — the CxCall
	// credential binds this method path + body, verified by the server's
	// per-call XSP-AUTH gate (the bearer plane is retired). No identity =
	// anonymous (floor: data ops open, admin denied), no auth header.
	if rb.seed.len == 32 {
		call_path := '/cxstore.v1.CxStore/${method}'
		if cred := grpc_call_auth_header(rb.did, rb.seed, call_path, msg, '') {
			hdrs << HpackHeader{'authorization', cred}
		}
	}
	mut req := []u8{}
...
fn grpc_client_status_err(op string, hash string, res GrpcClientResult) cx.Node {
	...
	match num {
		...
		1114 { return mk_err('cx-err:CXER1114', 'E_STORE_REF_CONFLICT: grpc ${op}: ${detail}') }
		else {}
	}
	// Cross-transport error identity (G13, cxstore-grpc.md §3): an EXACT CXER
	// in the cx-err-code trailer with no store-specific translation above
	// surfaces VERBATIM instead of collapsing to the coarse grpc-status (the
	// L99 planar-query lanes ride this — CXER0120 membership, CXER0271 caps,
	// CXER4700 authz, CXER4100 parse — matching the XSP error frame exactly).
	if num > 0 && res.cx_err.starts_with('cx-err:CXER') {
		return mk_err(res.cx_err, 'grpc ${op}: ${detail}')
	}
...
// grpc_client_put_blob / grpc_client_get_blob — the F1' OPAQUE pair over the
// gRPC edge (S6.5, G13 parity; cxstore-grpc.md §2). The Put/Get message shapes
// carry the raw bytes with encoding "raw"; PutBlob's reply hash field is the
// blob KEY; GetBlob absence surfaces the blob surface's CXER1121 contract.
fn grpc_client_put_blob(rb &RemoteBackend, raw string) cx.Node {
	res, errn, ok := grpc_client_call(rb, 'PutBlob', pb_encode_put_request(GrpcPutRequest{
		store:    rb.dir
		body:     raw.bytes()
		encoding: 'raw'
	}))
	...
	if res.grpc_status != 0 {
		return grpc_client_status_err('put-blob', '', res)
	}
	if res.messages.len == 0 {
		return mk_err('cx-err:CXER1101', 'E_STORE_BACKEND_UNREACHABLE: grpc put-blob: empty response')
	}
	m := pb_decode_put_response(res.messages[0]) or {
		return mk_err('cx-err:CXER1101', 'E_STORE_BACKEND_UNREACHABLE: grpc put-blob: malformed PutResponse')
	}
	return store_str(m.hash)
}

fn grpc_client_get_blob(rb &RemoteBackend, key string) cx.Node {
	res, errn, ok := grpc_client_call(rb, 'GetBlob', pb_encode_get_request(GrpcGetRequest{
		store: rb.dir
		hash:  key
	}))
	if !ok {
	...
	if res.grpc_status == 5 {
		return mk_err('cx-err:CXER1121', 'E_STORE_NOT_FOUND: ${key}')
	}
	if res.grpc_status != 0 {
		return grpc_client_status_err('get-blob', key, res)
	}
	if res.messages.len == 0 {
		return mk_err('cx-err:CXER1101', 'E_STORE_BACKEND_UNREACHABLE: grpc get-blob: empty response')
	}
	m := pb_decode_get_response(res.messages[0]) or {
		return mk_err('cx-err:CXER1101', 'E_STORE_BACKEND_UNREACHABLE: grpc get-blob: malformed GetResponse')
	}
	return store_str(m.body.bytestr())
}
...
// grpc_client_query collects the L97 flat-relation tuples: each QueryRow.row
// is a rendered [result doc= source= MATCH] tuple, forwarded verbatim (no
// reassembly — the wire frame IS the executor's tuple).
...
fn (t &GrpcObjWireTransport) send(op string, query string, body string) (int, string, bool) {
	...
	if res.grpc_status != 0 {
		// Error identity across transports (#194 / the G13 families): map the
		// trailer's exact CXER — falling back to the coarse grpc-status — onto
		// the SAME statuses the profile transport surfaces (xcl_ow_status:
		// not-found→404, ref-conflict→409, authz→403, rate→429), so the store
		// arms above this transport re-derive IDENTICAL store codes on both
		// wires. The old collapse-to-500 turned a target-absent aliases-set
		// into CXER1101 on this edge while the profile said CXER1121.
		num := grpc_cxer_num(res.cx_err)
		st := match num {
			1121, 1721, 1710 { 404 }
			1114 { 409 }
			1131, 1702, 1703 { 403 }
			1132, 1705, 1706, 4713 { 429 }
			else {
				match res.grpc_status {
					5 { 404 }
					10 { 409 }
					7, 16 { 403 }
					8 { 429 }
