...
struct WireResp {
	...
	sse_topic string
	// SSE-1 (xsp.md §4.1): the subscription negotiated the XSP envelope —
	// each event's `data:` field is base64(XSP event frame) instead of the
	// plain CX text. Carriage only; the framed payload is byte-identical to
	// what the plain lane delivers.
	sse_xsp bool
}
...
//
// Model (http.md §14, graduated from xap_architecture §11): reactors own sockets — accept, read, parse,
...
mut:
	...
	hdrs []WireHeader
	fd   int
	// #875 (http.md §13): the h2/TLS lane. When h2c is set the job came off
	// an HTTP/2 connection's owner thread, not a picoev reactor: fd is -1,
	// none of the per-fd flag/pipeline machinery applies, and the executor
	// completes the job through h2s_execute_job (response frames or SSE
	// promotion on stream h2_stream). Same bounded pool, same shed contract.
	h2c       &H2ServeConn = unsafe { nil }
	h2_stream u32
}
...
fn cx_dispatch_finish(fd int, keep_hold bool) ?DispatchJob {
	...
	if f & 2 != 0 {
		...
		// #873 (the 4-fast-clicks hang): when the completed job was an SSE
		// promotion, the fd is ALREADY in a subscriber set — the close must
		// purge every registry before the socket closes, or the OS recycles
		// the number onto a fresh connection that then receives other
		// visitors' frames and answers nothing. This exact race is a
		// 1-second-navigation browser's steady state: every page opens
		// /stream and the FIN lands while its dispatch is still in flight.
		cx_disp_pending.delete(fd)
		C.atomic_store_byte(&cx_disp_flags[fd], 0)
		cx_disp_mu.unlock()
		picoev.cx_notify_sse_close(fd)
		picoev.cx_release_fd(fd)
...
fn cx_dispatch_executor(id int) {
	...
	for {
		...
		for {
			...
			// #875: an h2-lane job completes through the connection's frame
			// writer — no fd flags, no per-fd pipelining (h2 streams are
			// concurrent by construction). Same dispatch, same pool.
			if job.h2c != unsafe { nil } {
				h2s_execute_job(mut job)
			...
			if w.sse {
				...
				if w.sse_topic != '' {
					// SSE-1: an XSP-envelope subscription registers under the
					// topic's envelope sibling key — same topic, negotiated
					// carriage; the publish fans out to both keys.
					cx_sse_topic_subscribe(sse_topic_key(w.sse_topic, w.sse_xsp),
						job.fd, prelude + w.body)
				} else {
...
fn dispatch_request(mut h ListenerHandler, raw_method string, raw_path string, raw_body string, xsp_hdrs XspReqHdrs, wire_hdrs []WireHeader) WireResp {
	...
	res, path_params := match_resource(svc, method, path) or {
		if method == 'HEAD' {
			...
			return invoke_handler(svc, r2, p2, 'HEAD', path, raw_body, query_nodes, wire_hdrs,
				mut h)
		}
		return mk_wire(404, svc.default_headers, 'not found: ${method} ${path}\n')
	}
	return invoke_handler(svc, res, path_params, method, path, raw_body, query_nodes,
		wire_hdrs, mut h)
}
...
//
// `wire_hdrs` reaches `$request/headers` for the same reason (#838, RULED):
// §10.3.3's request shape states the `headers` child normatively too, and the
// directive lane left it empty. The original rationale — that a directive
// service's auth rides the `[auth …]` seam — explains why headers were not
// NEEDED for auth; it does not make a normative field unpopulated by design. A
// session cookie is a header, so actor stamping cannot see its own principal
// without this.
fn invoke_handler(svc &ServiceRecord, res ResourceRecord, path_params []cx.Node,
	method string, path string, raw_body string, query_nodes []cx.Node,
	wire_hdrs []WireHeader, mut h ListenerHandler) WireResp {
	...
	env.cow_bindings()
	// #838 — the request BODY reaches the handler on this lane too.
	//
	// `dispatch_request` had `raw_body` in scope and this call passed
	// `?cx.Node(none)`, so `$request/body` was absent for every request served
	// over a real socket: no form submission, no command post, no JSON body
	// reachable from a directive-defined service. code.md §10.3.3 states the
	// request shape normatively and says `body` holds the parsed body when
	// `consumes=` permits — the default is `*/*`, so every resource permits it.
	//
	// EMPTY BODY STAYS ABSENT, deliberately. The in-process lane passes
	// `opts.payload`, which is `none` when there is no payload, and
	// build_request_node renders absence as an empty `[body]`. Forwarding `''`
	// as a string scalar would make a GET read `[body '']` here and `[body ()]`
	// there — a NEW divergence of exactly the kind this fixes. So absence maps
	// to absence and only a real body becomes a scalar.
	mut body_arg := ?cx.Node(none)
	if raw_body.len > 0 {
		body_arg = ?cx.Node(cx.ScalarNode{
			value:     cx.ScalarValue(raw_body)
			data_type: cx.ScalarType.string_type
		})
	}
	...
	env.bindings['request'] = build_request_node(method, path, path_params, body_arg,
		hdr_nodes, query_nodes)
	if svc.root != '' {
...
fn cx_sse_topic_publish(topic string, frame string) int {
	...
	// #875 (http.md §13.3): fan out to the TLS-transport subscribers too —
	// h2 streams (frames enqueued under per-stream flow control) and TLS-h1
	// feeds (sink queues). Enqueue-only on both: a slow consumer stalls its
	// own stream/connection, never this publisher.
	delivered += cx_sse_h2_topic_publish(topic, frame)
	delivered += cx_sse_tls1_topic_publish(topic, frame)
	return delivered
}
...
fn cx_response_to_wire(node cx.Node, defaults []cx.Attribute) WireResp {
	...
	if node is cx.Element {
		...
		if sub.name == 'sse-subscribe' {
			...
			// SSE-1 (xsp.md §4.1): an `[envelope codec="xsp"]` child opts THIS
			// subscription into XSP-envelope carriage — mirroring the upstream
			// POST's opt-in vocabulary. An unknown codec refuses the promotion
			// loudly; a silent plain fallback would hand an XSP-decoding client
			// undecodable frames.
			mut xsp_env := false
			for it in sub.items {
				if it is cx.Element && (it as cx.Element).name == 'envelope' {
					codec := (it as cx.Element).attr('codec')
					if codec != 'xsp' {
						return mk_wire(500, defaults, 'sse-subscribe envelope: unknown codec "${codec}" — the v1 web binding negotiates codec="xsp" (xsp.md §4.1)\n')
					}
					xsp_env = true
				}
			}
			...
			for it in sub.items {
				if it is cx.Element && (it as cx.Element).name == 'event' {
					...
					// the initial frame rides the subscription's negotiated carriage
					if xsp_env {
						frame = sse_frame_event_xsp(it as cx.Element) or {
							return mk_wire(500, defaults, 'sse-subscribe initial event refused XSP framing\n')
						}
					} else {
...
fn start_http_listener(mut rec ServiceRecord, mut env MatchEnv) ! {
	...
	// #28/W26 (found live by the studio's layout broadcast): the generic-topic
	// SSE close hook was registered ONLY on the handler-listener path, so on
	// THIS path — [?http-service], i.e. every reference estate — a client that
	// dropped its /stream left its fd in cx_sse_topic_subs forever. The OS
	// recycles the number onto the next accepted connection and the next
	// [$http:sse-publish] writes the frame INTO THAT CONNECTION'S RESPONSE
	// (curl: "Received HTTP/0.9"). Registration is append-only and idempotent;
	// picoev's close_conn and the accept-time recycle guard both notify it.
	picoev.cx_set_sse_on_close(cx_sse_topic_on_close_fd)
	retain_listener_handler(h) // vgc root — picoev holds h only as voidptr (#57)
	if rec.tls_cert != '' {
		// #875 (http.md §13.1): a [tls cert= key=] child puts TLS + ALPN
		// ("h2", "http/1.1") on this port; the negotiated protocol selects
		// the framing. No [tls] → the cleartext picoev path below, exactly
		// as before (no h2c — §13.1).
		start_tls_service_listener(mut h, host, rec.port, rec.tls_cert, rec.tls_key)!
	} else {
...
fn start_handler_listener(handler cx.Node, host string, port int, block bool, tls_cert string, tls_key string, mut env MatchEnv) !cx.Node {
	...
	retain_listener_handler(h) // vgc root — picoev holds h only as voidptr (#57)
	if tls_cert != '' {
		// #875 (http.md §13.1): tls:// serve — TLS + ALPN ("h2", "http/1.1").
		start_tls_service_listener(mut h, host, port, tls_cert, tls_key)!
	} else {
		spawn_shared_reactors(mut h, host, port)!
	}
	bind_host := if host == '' { '0.0.0.0' } else { host }
	scheme := if tls_cert != '' { 'tls' } else { 'tcp' }
	url := '${scheme}://${bind_host}:${port}'
	if block {
