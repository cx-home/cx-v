...
fn store_cxpack_backend(mut ms MemStore) ! {
	...
	if ms.enc_key_id != '' && ms.obj_pack_enc == unsafe { nil } {
		kms := store_kek_kms(ms.enc_key_id, ms.root)!
...
fn store_cxpack_compact(mut ms MemStore) ! {
	...
	// Erased tombstone objects (explicit, the alias-name treatment): the E
	// records reference them and attribution must survive compaction (§7b.1).
	for h in ms.erased_order {
		tomb := ms.erased[h] or { continue }
		tb := tomb.bytes()
		tk := cxstore.object_name(tb).hex()
		if tk !in name_hashes {
			payloads << tb
			name_payloads << tb
			name_hashes[tk] = true
		}
	...
	ms.log_records = ms.doc_order.len + ms.alias_order.len + ms.erased_order.len
}
	...
	if ms.enc_key_id != '' {
		...
		for hx, env in objs {
			...
			payload := ms.obj_pack_enc.open_envelope(key, env) or {
				// W5 (M29): a subject envelope under an ABSENT `sek/` key
				// cannot be judged HERE — the erasure records that evidence a
				// lawful shred replay AFTER this slurp — so it defers to the
				// pass-2 reconciliation (covered → a finding; uncovered → the
				// loud open refusal). Everything else (wrong KEK, tampered
				// bytes) stays the hard fail-closed open error.
				if penv := cxstore.parse_envelope(env) {
					if penv.key_id.starts_with(cxstore.sek_id_prefix)
						&& cxstore.is_key_unavailable(err.msg()) {
						continue
					}
				}
...
fn store_cxpack_replay(mut ms MemStore, mp string, verify_docs bool) ! {
	...
	defer {
		if !load_ok {
			...
			ms.alias_order = []
			ms.erased = map[string]string{}
			ms.erased_order = []
			ms.erased_roots = map[string]string{}
		}
	}
	...
	for ri, parts in recs {
		...
		match parts[0] {
			'C' {
				...
				return error('cxpack manifest ${mp}: legacy code record ${parts[1]} — the computation-identity-keyed store form is retired (F1-prime/A3); re-ingest the source through put-blob')
			}
			'D', 'B' {
				// 'B' = F1' opaque blob (raw bytes, key = hash(raw), rehash-VERIFIED
				// in pass 2 — never presence-only).
				store_hash := parts[1]
				...
				if parts[0] == 'B' {
					ms.blob_kind[store_hash] = true // F1' opaque blob
				}
			...
			'E' {
				// W5 erased-tombstone record (xsp store profile §7b.1): records
				// the attributed erasure; the doc's removal rode its own T line
				// in the same flush. A missing tombstone object is genuine
				// corruption — erasure evidence must never silently vanish.
				obj := hex.decode(parts[2]) or {
					if is_last {
						torn = true
						break
					}
					return error('cxpack manifest ${mp}: bad erased tombstone object hash: ${err.msg()}')
				}
				if obj.len != 32 {
					if is_last {
						torn = true
					...
					return error('cxpack manifest ${mp}: erased tombstone hash is ${obj.len} bytes, not 32')
				}
				payload := getter(obj) or {
					return error('cxpack ${ms.root}: erased tombstone object missing from pack: ${parts[2]}')
				}
	...
	for h, root in ms.obj_roots {
		if h in ms.blob_kind {
			// F1' opaque blob: verify key == hash(raw) — never presence-only.
			payload := getter(root) or {
				return error('cxpack ${ms.root}: blob object missing from pack for ${h}')
			}
			rehash := cx.cx_tag_address(cx.cx_default_hash_algo, sha256.sum256(payload).hex())
			if rehash != h {
				return error('cxpack ${ms.root}: blob ${h} rehashes to ${rehash}')
			}
			continue
		}
		if store_whole_doc_root(h, root) {
			// Stream 20: whole-doc object (subject-bearing) — the getter
			// self-verifies the hash, so presence is the check. A failed
			// open reconciles from evidence (M29/W5): a doc covered by a
			// journaled erase record is lawfully shredded — a finding, not
			// a load fault (the store opens; the read surface answers the
			// typed CXER1145) — while an UNCOVERED failure keeps the loud
			// open refusal, fail-closed (outage, misconfiguration, or
			// unlawful destruction must never load as clean).
			getter(root) or {
				msg := store_shredded_read_err(mut ms, h, root)
				if msg.contains('E_STORE_SHREDDED') {
					continue
				}
				return error('cxpack ${ms.root}: ${msg}')
			}
			continue
		}
