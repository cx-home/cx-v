...
fn test_porcelain_clone_into_empty() {
	...
	g1 := pc_get(dst, k1)
	g2 := pc_get(dst, k2)
	assert g1 is cx.ScalarNode && sw_scalar(g1).contains('[id 1]'), 'cloned doc1 missing'
	assert g2 is cx.ScalarNode && sw_scalar(g2).contains('[id 2]'), 'cloned doc2 missing'
...
fn test_porcelain_push_pull_local() {
	...
	gp := pc_get(remote, k)
	assert gp is cx.ScalarNode && sw_scalar(gp).contains('pushed'), 'pushed doc not on remote'
	...
	gl := pc_get(local2, k2)
	assert gl is cx.ScalarNode && sw_scalar(gl).contains('pulled'), 'pulled doc not local'
}
...
fn test_porcelain_push_cross_tier() {
	...
	// the wire).
	gd := pc_get(daemon, k)
	assert gd is cx.ScalarNode && sw_scalar(gd).contains('[n 42]'), 'daemon missing pushed doc'
...
fn test_porcelain_gc_retains_shared_objects() {
	...
	gb := pc_get(s, kb)
	assert gb is cx.ScalarNode && sw_scalar(gb).contains('[id 2]'), 'gc collected a shared subtree — B is broken'
...
fn test_porcelain_branch_cas_and_force() {
	...
	assert sw_scalar(ga) == h2, 'branch-force must move main → h2'
