...
import code {
	...
	crypto_stdlib_builtin,
	mk_err,
	module_loader_scan_directives,
	time_stdlib_builtin,
}
...
const xap_err_pkg_schema_reinterprets = 'cx-err:CXER4890' // E_XAP_PKG_SCHEMA_REINTERPRETS (RULED: SEA-1)
const xap_err_pkg_coverage_gap = 'cx-err:CXER4891' // E_XAP_PKG_COVERAGE_GAP (RULED: SEA-1)
	...
	if hashes.len == 0 {
		...
		for r in xap_gc_flatten(q) {
			if r is cx.Element && r.name == 'result' {
				// L97 flat relation: `doc=` carries the containing document's
				// content address (one tuple per match — the `!in` dedup below
				// already collapses multi-match docs to one hash).
				h := xap_elem_attr(r, 'doc')
				if h != '' && h !in hashes {
					hashes << h
				}
			}
...
fn xap_pkg_sign(args []cx.Node) ?cx.Node {
	...
	// I1 stream 19 (L36 rule 2): the provenance suite slot is an ATOM —
	// type-strict, closed-vocabulary, never confusable with a data string.
	items << xap_elem('signature', [cx.new_attribute('alg', cx.ScalarValue('ed25519'),
		cx.AttributeMeta{ data_type: ?string('atom') }), xap_attr('key', 'key-1'),
...
fn xap_pkg_publish(args []cx.Node) ?cx.Node {
	...
	// Schema-lineage stage (RULED: SEA-1, §6 step 3): against the previous
	// released version, changed schema entries classify via schema.md §16.5 —
	// derivable changes derive their claim; a reinterpreting change uncovered
	// by an authored in-tree claim refuses CXER4890 BEFORE anything stages.
	lerr, lhas_err, lchildren := xap_pkg_publish_lineage_stage(args[0], name, version, m)
	if lhas_err {
		return lerr
	}
	alias := '${name}@${version}'
...
// ── the publish-time schema-lineage stage (RULED: SEA-1, §6 step 3) ─────────

// xap_pkg_schema_entries: path → text for every `*.cxs` tree entry.
fn xap_pkg_schema_entries(tree cx.Element) map[string]string {
	mut out := map[string]string{}
	for it in tree.items {
		if it is cx.Element && it.name == 'entry' {
			p := xap_elem_attr(it, 'path')
			if p.ends_with('.cxs') {
				out[p] = xd_text_content(it)
			}
		}
	}
	return out
...
// xap_pkg_authored_lineage_claims collects [schema-lineage] claims authored
// into the package tree (entries whose path ends `lineage.cx` — SEA-1d).
fn xap_pkg_authored_lineage_claims(tree cx.Element) []cx.Element {
	mut out := []cx.Element{}
	for it in tree.items {
		if it is cx.Element && it.name == 'entry'
			&& xap_elem_attr(it, 'path').ends_with('lineage.cx') {
			doc := cx.parse(xd_text_content(it)) or { continue }
...
// xap_pkg_claim_endpoint reads a claim's [from]/[to] child text.
fn xap_pkg_claim_endpoint(c cx.Element, which string) string {
	for it in c.items {
		if it is cx.Element && it.name == which {
			if it.items.len > 0 {
				v := it.items[0]
...
// xap_pkg_prev_version: the highest released version of `name` below
// `version` — ('', '') when none. Lineage aliases (`+lineage`) never count.
fn xap_pkg_prev_version(store cx.Node, name string, version string) (string, string) {
	new_key := xap_pkg_semver_key(version)
	mut best := ''
	mut best_key := ''
	mut best_hash := ''
	aliases := xd_store('store-list-aliases', [store])
	for a in xap_gc_flatten(aliases) {
		if a is cx.Element && a.name == 'alias' {
			...
			if !an.starts_with('${name}@') || an.contains('+') {
				continue
			}
			v := an.all_after('${name}@')
			k := xap_pkg_semver_key(v)
			if k < new_key && (best == '' || k > best_key) {
				best = v
				best_key = k
				best_hash = xap_elem_attr(a, 'hash')
			}
		}
	}
...
// xap_pkg_publish_lineage_stage diffs the new version's schema entries
// against the previous released version's (path-matched), derives Lane-2
// [schema-lineage] claims for derivable changes (via cx.schema_compat,
// schema.md §16.5), accepts authored in-tree claims covering a
// reinterpreting pair, and otherwise refuses CXER4890 carrying the
// classifier's [missing-rule …] prompts. Accepted claims are stored as one
// lineage document aliased `NAME@VERSION+lineage` (SEA-1d). Returns
// (err, has_err, result-children).
fn xap_pkg_publish_lineage_stage(store cx.Node, name string, version string, m cx.Element) (cx.Node, bool, []cx.Node) {
	prev_v, prev_mhash := xap_pkg_prev_version(store, name, version)
	if prev_v == '' {
		return jrn_null(), false, []cx.Node{}
	}
	new_tree := xap_pkg_tree_of(store, m) or { return jrn_null(), false, []cx.Node{} }
	prev_mdoc := xd_store('store-get-doc', [store, cx.Node(xap_str(prev_mhash))])
	prev_m := xd_elem_of(prev_mdoc, 'package') or { return jrn_null(), false, []cx.Node{} }
	prev_tree := xap_pkg_tree_of(store, prev_m) or { return jrn_null(), false, []cx.Node{} }
	old_schemas := xap_pkg_schema_entries(prev_tree)
	new_schemas := xap_pkg_schema_entries(new_tree)
	authored := xap_pkg_authored_lineage_claims(new_tree)
	mut paths := []string{}
	for p, _ in old_schemas {
		if p in new_schemas {
			paths << p
		}
	}
	...
	mut authored_n := 0
	for p in paths {
		old_text := old_schemas[p]
		new_text := new_schemas[p]
		if old_text == new_text {
			continue
		}
		rep := cx.schema_compat(old_text, new_text, cx.SchemaCompatOpts{}) or {
			continue // not a loadable schema pair — nothing to classify
		}
		if rep.verdict == 'identical' {
			continue
		}
		if rep.verdict == 'derivable' {
			tdoc := cx.parse(rep.translator) or { continue }
			for n in tdoc.elements {
				if n is cx.Element && n.name == 'schema-lineage' {
					claims << cx.Node(n)
					derived_n++
				}
		...
		// refused — an authored claim covering exactly this pair admits it.
		mut covered := false
		for ac in authored {
			if xap_pkg_claim_endpoint(ac, 'from') == rep.from_addr
				&& xap_pkg_claim_endpoint(ac, 'to') == rep.to_addr {
				claims << cx.Node(ac)
				authored_n++
				covered = true
				break
		...
		for c in rep.changes {
			if c.prompt != '' {
				mut mattrs := [xap_attr('class', c.class), xap_attr('type', c.type_name)]
				if c.field != '' {
					mattrs << xap_attr('field', c.field)
				}
				mattrs << xap_attr('prompt', c.prompt)
				missing << cx.Node(xap_elem('missing-rule', mattrs, []))
			}
		}
		return xap_gc_err(xap_err_pkg_schema_reinterprets, 'E_XAP_PKG_SCHEMA_REINTERPRETS: schema ${p} changed ${prev_v} -> ${version} in a way that reinterprets data — publish refused; each [missing-rule] names the rule that would admit it (author the claim in a `lineage.cx` tree entry, or make the change derivable — schema.md §16.5, RULED: SEA-1)',
			[xap_attr('path', p), xap_attr('from', rep.from_addr),
			xap_attr('to', rep.to_addr)], missing), true, []cx.Node{}
	}
	if claims.len == 0 {
		return jrn_null(), false, []cx.Node{}
	}
	ldoc := xap_elem('lineage', [xap_attr('package', name), xap_attr('version', version)],
		claims)
	lput := xd_store('store-put-doc', [store, cx.Node(ldoc)])
	if xd_is_err(lput) {
		return lput, true, []cx.Node{}
	}
	lhash := xd_str_of(lput)
	laset := xd_store('store-set-alias', [store, cx.Node(xap_str('${name}@${version}+lineage')),
		cx.Node(xap_str(lhash))])
	if xd_is_err(laset) {
		return laset, true, []cx.Node{}
	}
	return jrn_null(), false, [
		cx.Node(xap_elem('lineage', [xap_attr('doc', lhash),
			xap_attr('claims', claims.len.str()), xap_attr('derived', derived_n.str()),
			xap_attr('authored', authored_n.str())], [])),
	]
}
...
fn xap_pkg_verify_manifest(store cx.Node, m cx.Element, opts cx.Node) ?cx.Node {
	...
	// I1 stream 19 (L36/#702): pkg-verify is a REGISTRY LOOKUP, not a
	// hardcoded equality — unknown/reserved suites fail closed.
	cx.cx_suite_verify_gate(xap_elem_attr(sig_el, 'alg')) or {
		return xap_gc_err(xap_err_pkg_sig_invalid, 'E_XAP_PKG_SIG_INVALID: ${err.msg()}', [], [])
	}
...
// present among the tree's top-level [?def]s; a declared computation-identity
// identity= claim must match the code's (recomputed via the ONE pure
// relation, never a parallel hasher — A4).
...
// declared computation-identity `identity=` CLAIM must match the code's —
// recomputed via the ONE pure relation (cx_code_tier2_hash, the same fn
// behind [$cx:computation-id]; A4, 2026-08-08 — the retired scratch-store
// put-def detour is gone). The claim is spelled `computes-as:<algo>:<hex>`
// (A1: a distinct token, never a parseable address).
fn xap_pkg_exports_check(tree cx.Element, exports cx.Element) ?cx.Node {
	...
	for ex in xap_gc_children(exports, 'def') {
		...
		if want != '' {
			hash := cx_code_tier2_hash(def_src[local]) or {
				return xap_gc_err(xap_err_pkg_gate_rejected, 'E_XAP_PKG_GATE_REJECTED: def "${dn}" computation identity cannot be derived: ${err.msg()}',
					[xap_attr('stage', 'exports'), xap_attr('def', dn)], [])
			}
			got := 'computes-as:${hash}'
			if got != want {
				return xap_gc_err(xap_err_pkg_gate_rejected, 'E_XAP_PKG_GATE_REJECTED: def "${dn}" computation identity ${got} does not match the declared ${want}',
					[xap_attr('stage', 'exports'), xap_attr('def', dn)], [])
			}
		}
	}
...
// ── the install-time journal-coverage gate (RULED: SEA-1, L151) ─────────────

// xd_err_code: the code= of an err value ('' when not an err).
fn xd_err_code(n cx.Node) string {
	if n is cx.Element {
		if n.name == 'err' {
			return xap_elem_attr(n, 'code')
		}
	}
	return ''
}

// xap_pkg_collect_declared tallies the distinct declared schema= addresses
// (journal.md §2.10) across one stream's entries into `declared`.
fn xap_pkg_collect_declared(journal cx.Node, stream string, mut declared map[string]int) ?cx.Node {
	mut args := [journal, jrn_int(1)]
	if stream != '' {
		args << jrn_str(stream)
	}
	entries := journal_stdlib_builtin('journal-since', args) or {
		return mk_err(xap_err_pkg_invalid, 'E_XAP_PKG_INVALID: opts.journal is not a readable journal handle')
	}
	if xd_is_err(entries) {
		return entries
	}
	for n in xap_gc_flatten(entries) {
		if n is cx.Element && n.name == 'entry' {
			sch := jrn_entry_schema(n)
			if sch != '' {
				declared[sch]++
			}
		}
...
// xap_pkg_coverage_gate — the L151 coverage pre-flight ("is every event kind
// in the journal covered by the current fold+upcasters?") productized at
// install (RULED: SEA-1): a pure query over (declared schema= addresses) ×
// (the package's lineage graph). Scope (SEA-1e): an address is IN SCOPE when
// it appears in the package's history — a published version's schema
// content-hash, a lineage-claim endpoint (every `NAME@*+lineage` document,
// plus OPTS.lineage claims), or a current address; each in-scope non-current
// address must admit the unique lineage path to a current address, else the
// install refuses CXER4891 naming every uncovered address and its entry
// count. Entries declaring no schema are out of scope.
fn xap_pkg_coverage_gate(store cx.Node, m cx.Element, journal cx.Node, opts cx.Node) ?cx.Node {
	name := xap_elem_attr(m, 'name')
	tree := xap_pkg_tree_of(store, m) or { return none } // no tree → nothing declared
	mut current := map[string]bool{}
	for _, text in xap_pkg_schema_entries(tree) {
		h := cx.schema_content_hash(text) or { continue }
		current['sha2-256:' + h.hex()] = true
	}
	...
	for a in xap_gc_flatten(aliases) {
		if a is cx.Element && a.name == 'alias' {
			an := xap_elem_attr(a, 'name')
			if !an.starts_with('${name}@') {
				continue
			}
			h := xap_elem_attr(a, 'hash')
			if an.ends_with('+lineage') {
				ldoc := xd_store('store-get-doc', [store, cx.Node(xap_str(h))])
				if le := xd_find_elem(ldoc, 'lineage') {
					for c in le.items {
						if c is cx.Element && c.name == 'schema-lineage' {
							f := xap_pkg_claim_endpoint(c, 'from')
							t := xap_pkg_claim_endpoint(c, 'to')
							key := '${f} -> ${t}'
							if key in seen_edge {
								continue
							}
			...
			// a released version: its schema addresses are package history.
			vmdoc := xd_store('store-get-doc', [store, cx.Node(xap_str(h))])
			vm := xd_elem_of(vmdoc, 'package') or { continue }
			vtree := xap_pkg_tree_of(store, vm) or { continue }
			for _, text in xap_pkg_schema_entries(vtree) {
				vh := cx.schema_content_hash(text) or { continue }
				in_scope['sha2-256:' + vh.hex()] = true
			}
		}
	}
	if extra := xap_map_get_node(opts, 'lineage') {
		for n in xap_gc_flatten(extra) {
			if n is cx.Element && n.name == 'schema-lineage' {
				f := xap_pkg_claim_endpoint(n, 'from')
				t := xap_pkg_claim_endpoint(n, 'to')
				key := '${f} -> ${t}'
				if key in seen_edge {
					continue
				}
	...
	// declared addresses: the default chain + every named stream.
	mut declared := map[string]int{}
	if derr := xap_pkg_collect_declared(journal, '', mut declared) {
		return derr
	}
	streams := journal_stdlib_builtin('journal-streams', [journal]) or { jrn_empty() }
	if !xd_is_err(streams) {
		for sn in xap_gc_flatten(streams) {
			if sn is cx.Element && sn.name == 'stream' {
				sname := xap_elem_attr(sn, 'name')
				if sname != '' {
					if derr := xap_pkg_collect_declared(journal, sname, mut declared) {
						return derr
					}
				}
			}
	...
	curs.sort()
	mut uncovered := []cx.Node{}
	for a4 in addrs {
		if a4 in current {
			continue
		}
		if a4 !in in_scope {
			continue // another feature's vocabulary — out of this gate's scope
		}
		mut covered := false
		for cur in curs {
			r := jrn_lineage_path([jrn_seq(claims), jrn_str(a4), jrn_str(cur)]) or { continue }
			ecode := xd_err_code(r)
			if ecode == '' {
				covered = true
				break
			}
			if ecode != 'cx-err:CXER4644' {
				// ambiguous graph / malformed claims: fail-closed, loudly.
				return r
			}
		}
		if !covered {
			uncovered << cx.Node(xap_elem('uncovered', [xap_attr('address', a4),
				xap_attr('entries', declared[a4].str())], []))
		}
	}
	if uncovered.len > 0 {
		return xap_gc_err(xap_err_pkg_coverage_gap, 'E_XAP_PKG_COVERAGE_GAP: the journal holds event kinds the current fold+upcasters do not cover — each [uncovered] names a declared schema address with no lineage path to a current schema; publish the missing lineage (cx schema compat derives it for derivable changes) or supply opts.lineage (RULED: SEA-1)',
			[xap_attr('package', name)], uncovered)
	}
	return none
}
...
fn xap_pkg_install(args []cx.Node) ?cx.Node {
	...
	// journal-coverage gate (RULED: SEA-1, L151): when the installing
	// deployment supplies its journal (opts.journal), every declared schema=
	// address in scope of this package's lineage graph must be current or
	// admit the unique lineage path to a current address — else CXER4891.
	// Without a journal there is no history to check (the gate is vacuous).
	if jn := xap_map_get_node(opts, 'journal') {
		if cerr := xap_pkg_coverage_gate(store, m, jn, opts) {
			return cerr
		}
	}
