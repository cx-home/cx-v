...
//   / 4812 anonymous-refused (attach-guest under a policy with no floor,
//   GA-1). Cancellation is the core CXER0260; capability denial the core
//   CXER0271 (both inherited, not session codes).
...
const session_err_anonymous_refused = 'cx-err:CXER4812' // E_SESSION_ANONYMOUS_REFUSED (GA-1, §4.7)
...
mut:
	...
	name string
	// Guest marker (GA-1 / #857): true for a session established by
	// attach-guest at the deployment's anonymous floor (xap_identity_model
	// §4.7) — proofless, per-visitor independent (never mirrored by subject),
	// and the one binding a later PROVEN attach may retire via the
	// remint-on-privilege-transition seam (session_rebind_guard).
	guest bool
...
fn session_materialize(rec &SessionRecord) cx.Node {
	...
	// A guest (anonymous-floor) session surfaces the marker so surfaces and
	// audit read the pre-login state directly (GA-1b / §4.7).
	if rec.guest {
		attrs << session_str_attr('guest', 'true')
	}
	mut items := []cx.Node{}
...
// session. `guest` (GA-1b) classifies as cookie-shaped: a guest session's
// only credential is the ambient session cookie, so it is CSRF-guarded and
// rotate-eligible exactly like a cookie attach — never CSRF-exempt.
fn session_via_of(rec &SessionRecord) string {
	mut via := ''
	for c in rec.clients {
		if c.via == 'cookie' || c.via == 'guest' {
			return 'cookie'
		}
		if via == '' {
			via = c.via
...
fn session_stdlib_builtin(name string, args []cx.Node) ?cx.Node {
	...
	match name {
		...
		'session-attach-guest' {
			return session_attach_guest_impl(args)
		}
		'session-list' {
...
fn session_rebind_guard(req cx.Node, token string, cfg map[string]cx.Node) ?cx.Node {
	...
	// GA-1 remint-on-privilege-transition (§4.7 / §2.8.4, #857): the cookie
	// names a live GUEST session and the new token has VERIFIED and mapped —
	// the anonymous→proven transition. The pre-login id MUST NOT survive it
	// (fixation defense at the login seam), so the guest session is
	// invalidated server-side here and the pipeline establishes the proven
	// session under a freshly minted id. Fail-closed ordering holds: a token
	// that failed verify/map returned `none` above, leaving the guest
	// session (and the visitor's pre-login state) intact.
	if existing.guest {
		session_guest_invalidate(existing.id)
		return none
	}
...
// session_guest_invalidate retires a live guest session server-side (GA-1):
// clients unindexed, state detached, record dropped — the pre-login id
// resolves to absence from here on. Guest sessions never hold
// by_subject/by_name entries (per-visitor independence), so no default
// pointers need clearing.
fn session_guest_invalidate(id string) {
	mut reg := session_reg()
	mut rec := reg.sessions[id] or { return }
	if !rec.guest {
		return
	}
	for c in rec.clients {
...
fn session_attach_xsp_impl(args []cx.Node) cx.Node {
	...
	// Read child-OR-attr: an EMPTY [attach] (sole-store shorthand) ATOMIZES
	// to `attach=null` through the data-bin frame carriage (the W2 rule's
	// degenerate case) — the attr form carries no tenant/selector, which is
	// exactly the shorthand's meaning (the listener resolved the sole mount
	// and passes it as cfg tenant).
...
// attach-guest $req $cfg — the FOURTH attach transport (GA-1 / #857): a
// session-establishing act for the principal that proves NOTHING — the
// deployment's anonymous floor (xap_identity_model §4.7). It mints the SAME
// [session] value through the SAME cookie adapter as attach-cookie (same
// HttpOnly/Secure/SameSite posture and CXER4810/4811 refusals, same
// per-session CSRF synchronizer, same attached|detached|expired states) and
// registers it, so of/from-cookie/by-id resolve it like any other.
//   - POLICY GATE FIRST: cfg `anonymous-floor` names the floor principal;
//     its presence IS the deployment's policy admission (mirroring
//     attach-xsp's cfg). Absent → CXER4812 E_SESSION_ANONYMOUS_REFUSED, the
//     production-default common case (§4.7: policy either refuses or maps
//     to the floor). cfg `tenant` is required (a real (principal, tenant)
//     the PEP gates like any other — no anonymous commit by default).
//   - PER-VISITOR INDEPENDENCE: every guest attach mints an INDEPENDENT
//     session. N visitors share one floor subject but never one session —
//     no by_subject write, no mirror, never the floor's "default" session.
//   - IDEMPOTENT over a live guest cookie for the same (floor, tenant); a
//     live PROVEN cookie session refuses (GA-1a — no downgrade, the binding
//     is immutable). The anonymous→proven remint lives in
//     session_rebind_guard (the proven attach retires the guest id).
fn session_attach_guest_impl(args []cx.Node) cx.Node {
	if args.len < 2 {
		return mk_err(session_err_anonymous_refused, 'E_SESSION_ANONYMOUS_REFUSED: attach-guest expects (req, cfg)')
	}
	req := args[0]
	cfg := session_opts(args[1])
	// (policy) — refusal is the common case (§4.7, GA-1 property 3).
	floor := session_opt_str(cfg, 'anonymous-floor', '')
	if floor == '' {
		return mk_err(session_err_anonymous_refused, 'E_SESSION_ANONYMOUS_REFUSED: the deployment attach policy does not admit an anonymous floor — no `anonymous-floor` principal in cfg (xap_identity_model §4.7; refusing is the production default)')
	}
	tenant := session_opt_str(cfg, 'tenant', '')
	if tenant == '' {
		return mk_err(session_err_tenant_unresolved, 'E_SESSION_TENANT_UNRESOLVED: attach-guest cfg has no `tenant` (the floor is a real (principal, tenant), §4.7)')
	}
	// (transport) — the guest credential IS the cookie, so the cookie-side
	// TLS bright line applies (§2.8.3), plus the up-front flag refusals.
	allow_insecure := session_opt_bool(cfg, 'allow-insecure', false)
	if !session_request_is_tls(req) && !allow_insecure {
		return mk_err(session_err_cookie_insecure, 'E_SESSION_COOKIE_INSECURE_CONTEXT: cannot issue a Secure guest session cookie over a non-TLS request')
	}
	if e := session_cookie_flags_ok(cfg) {
		return e
	}
	// (existing cookie) — idempotence / no-downgrade (GA-1a).
	cookie_name := session_opt_str(cfg, 'cookie-name', '__Host-cxsid')
	mut cid := session_read_cookie(req, cookie_name)
	if cid == '' && cookie_name == '__Host-cxsid' {
		cid = session_read_cookie(req, 'cxsid')
	...
	if cid != '' {
		if rec := session_rec_by_id(cid) {
			if !rec.guest {
				return mk_err(session_err_rebind_refused, 'E_SESSION_REBIND_REFUSED: the request already carries a live proven session — attach-guest never downgrades an authenticated binding (§4.2)')
			}
			if rec.principal_id == floor && rec.tenant_id == tenant {
				// the visitor already holds a live guest session: return it
				// (resolve, not a second mint — the basket survives).
				return session_seq([session_materialize(rec),
					session_set_cookie_directive(rec, cfg)])
			}
			return mk_err(session_err_rebind_refused, 'E_SESSION_REBIND_REFUSED: the request carries a live guest session bound to a different (principal, tenant) — the binding is immutable (§4.2)')
		}
	}
	// (mint) — an independent per-visitor session at the floor, with the
	// claim-set shape the shared machinery reads (sub/tid, as attach-did).
	now_marker := session_opt_str(cfg, 'now', '')
	claims := cx.Element{
		name:  'claims'
		attrs: [
	...
	mut reg := session_reg()
	sid := session_mint_id()
	client_id := session_next_client_id(mut reg)
	mut rec := &SessionRecord{
		id:           sid
		state:        'attached'
		principal_id: floor
		...
		established:  now_marker
		now_secs:     session_cfg_now_secs(cfg)
		claims:       cx.Node(claims)
		csrf_token:   session_mint_csrf()
		clients:      [
			SessionClient{
				id:          client_id
				channel:     'http'
	...
	reg.sessions[sid] = rec
	reg.client_idx[client_id] = sid
	// NO reg.by_subject write: guest sessions never mirror (per-visitor
	// independence) and never become the floor subject's default session.
	return session_seq([session_materialize(rec), session_set_cookie_directive(rec, cfg)])
}
