...
pub fn content_type_for(filename string) string {
	...
	return match ext {
		...
		// CX's own formats (#705). `application/cx` and never `text/cx`:
		// a `text/*` intermediary may normalize line endings, and that
		// corrupts a content address (grammar_lexicon_review §5, the
		// media-type ruling). Without these rows [$http:serve-file] handed
		// out application/octet-stream for CX documents — the engine
		// failing to recognise its own data format over the wire.
		//   .cx  source / data document
		//   .cxd conformance-corpus document (787 uses in-tree)
		//   .cxs schema document
		'cx', 'cxd', 'cxs' { 'application/cx' }
		'pdf'             { 'application/pdf' }
		'zip'             { 'application/zip' }
