...
// sql_scheme extracts the DSN scheme. BOTH standard spellings are recognised
// (#520): the authority form `sqlite:///path` / `postgres://host/db`, and the
// opaque form `sqlite::memory:` / `sqlite:file.db` that every mainstream
// driver accepts for a non-network target. Reading only `://` made
// `sqlite::memory:` — the canonical in-memory DSN — resolve to an EMPTY
// scheme, which then produced the "rebuild with -d cx_db_sqlite" diagnostic:
// a malformed-looking URL sent the reader off to rebuild a binary whose
// engine was present all along.
fn sql_scheme(url string) string {
	if idx := url.index('://') {
		return url[..idx]
	}
	// opaque form: everything before the FIRST colon, when there is one and it
	// is not the leading character (a bare `:memory:` carries no scheme).
	if idx := url.index(':') {
		if idx > 0 {
			return url[..idx]
		}
...
// sql_known_schemes — the schemes CX has an engine FOR, independent of which
// engines this binary was built with. The split matters for the diagnostic:
// an unknown scheme is a URL mistake, a known scheme without its build define
// is a packaging matter, and no scheme at all is neither.
const sql_known_schemes = ['sqlite', 'postgres', 'postgresql', 'mysql']
...
fn sql_open_impl(args []cx.Node) cx.Node {
	...
	// Three distinct conditions, three distinct diagnoses (#520). The single
	// conflated message told a caller with a malformed URL to rebuild their
	// binary, and told a caller on an engine-free build the same thing it
	// told one who simply typo'd a scheme.
	if scheme == '' {
		return mk_err('cx-err:CXER1100', 'E_STORE_UNRESOLVED_BACKEND: sql-open needs a scheme — write `sqlite:///path/to.db`, `sqlite::memory:`, `postgres://host/db` or `mysql://host/db`; got ${url}')
	}
	if scheme !in sql_known_schemes {
		return mk_err('cx-err:CXER1100', 'E_STORE_UNRESOLVED_BACKEND: unknown SQL scheme "${scheme}" in ${url} (known: ${sql_known_schemes.join(', ')})')
	}
	return mk_err('cx-err:CXER1100', 'E_STORE_UNRESOLVED_BACKEND: this build carries no ${scheme} engine (scheme is valid; the binary was built without it — rebuild with -d cx_db_sqlite / -d cx_db_pg / -d cx_db_mysql, or check `cx -v` for the engines this binary has) in ${url}')
}
