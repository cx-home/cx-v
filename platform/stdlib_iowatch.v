...
mut:
	...
	// peeked stashes changes pulled by the delivery contract's
	// NON-CONSUMING readiness probe (#762): the receive arms drain it
	// before touching the event channel.
	peeked []WatchChange
...
fn watch_handle_element(id int, root string) cx.Node {
	return cx.Element{
		...
		attrs: [
			...
			// the delivery §4 contract attrs (additive; #762 V4): the
			// io.md rung verbatim — overflow means rescan; fs events are
			// deliver-once (nothing replays).
			cx.Attribute{
				name:  'rung'
			...
			cx.Attribute{
				name:  'sharing'
				value: cx.ScalarValue('independent')
			},
			cx.Attribute{
				name:  'flow'
				value: cx.ScalarValue('pull')
			},
			cx.Attribute{
				name:  'retention'
			...
			cx.Attribute{
				name:  'on-close'
				value: cx.ScalarValue('io-watch-close')
			},
		]
	}
}
...
// ── the watch-handle consumption arms (Ring2SubOps; #762 V2) ────────────────

// iowatch_sub_next — one event: the peek stash first, then watcher_next.
fn iowatch_sub_next(mut w Watcher, timeout_ms i64) cx.Node {
	if w.peeked.len > 0 {
		ev := w.peeked[0]
		w.peeked.delete(0)
		return watch_change_element(ev)
	}
	return watcher_next(mut w, timeout_ms)
}

fn iowatch_sub_receive(sub cx.Node, max int, deadline_ms i64, mut env MatchEnv) cx.Node {
	_ = env
	if d := cap_guard('read', 'watch receive') {
		return d
	}
	id := watch_id_of(sub) or {
		return mk_err('cx-err:CXER0100', 'E_OPERAND_KIND: expected a watch handle element')
	}
	mut w := watch_lookup(id) or { return mk_err('cx-err:CXER3409', 'E_IO_HANDLE_CLOSED: receive') }
	if max == 0 {
		// unbatched: what is there now (the io absence posture on quiet).
		return iowatch_sub_next(mut w, 0)
	}
	mut out := []cx.Node{}
	sw := vtime.new_stopwatch()
	for out.len < max {
		mut remaining := i64(0)
		if deadline_ms >= 0 {
			el := sw.elapsed().milliseconds()
			if el >= deadline_ms && out.len > 0 {
				break
			}
			remaining = if el >= deadline_ms { i64(0) } else { deadline_ms - el }
		}
		ev := iowatch_sub_next(mut w, remaining)
		if ev is cx.Element {
			if ev.name == 'change' {
				out << cx.Node(ev)
				continue
		...
		break // absence — quiet within the wait
	}
	return cx.Element{
		name:  code.seq_marker_name
		items: out
...
// iowatch_sub_ready — non-consuming: stash-peek the event channel.
fn iowatch_sub_ready(sub cx.Node, mut env MatchEnv) bool {
	_ = env
	id := watch_id_of(sub) or { return false }
	mut w := watch_lookup(id) or { return false }
	if w.peeked.len > 0 || stdatomic.load_u64(&w.pending_overflow) != 0 {
		return true
	}
	ev := watcher_next(mut w, 0)
	if ev is cx.Element {
		if ev.name == 'change' {
			op := ev.attr('op')
			mut wc := WatchChange{
...
// iowatch_ring2_builtin claims the §3.7 watch verbs on the I3 registry's
// env-free chain, carrying the read-path capability gate WITH the pack:
// the gate used to live in io_stdlib_builtin (Ring 1), but the registry
// probe precedes the io pack in the dispatch chain, so the pack gates
// itself fail-closed BEFORE any effect — identical denial value, same
// cap_guard. watch-close needs no capability (like io-close); the
// io-107 denial lane is the inner [$io:watch]'s read gate.
fn iowatch_ring2_builtin(name string, args []cx.Node) ?cx.Node {
	if name == 'io-watch' {
		// the constructor answers on the env-aware arm (#762 V4 — the
		// handle is closeable, and the stamp needs the program's
		// closeable registry): fall through.
		return none
	}
	if name == 'io-watch-next' {
		if d := cap_guard('read', name) {
			return d
		}
	}
	return iowatch_dispatch(name, args)
}

// iowatch_stdlib_builtin_env — the env-aware constructor arm (#762 V4):
// the returned watch handle is closeable (SAP §5.1; [?with-open]/[?close]
// share the stamp — the close IS watch-close, idempotent).
fn iowatch_env_ring2_arm(name string, args []cx.Node, mut env MatchEnv) ?cx.Node {
	if name != 'io-watch' {
		return none
	}
	if d := cap_guard('read', name) {
	...
	h := iowatch_dispatch(name, args) or { return none }
	if h is cx.Element {
		if h.name == 'watch' {
			wid := h.attr('handle').int()
			return bus_stamp_closeable(cx.Node(h), 'io-watch-close', fn [wid] () ! {
				watcher_close(wid)
			}, mut env)
		}
	}
