...
pub fn grpc_dispatch(call GrpcCall, mut state ServiceState, ctx ServeContext) GrpcReply {
	...
	path := call.headers[':path'] or { '' }
	op := grpc_op_from_path(path)
	...
	// S3 (RULED G1a): ONE per-call XSP-AUTH gate before any routing — the
	// profile's authority calculus, per call. The target store is proto
	// field 1 in EVERY request message (pb_decode_store_request skips the
	// rest), so the mount/tenant resolves generically; Capabilities carries
	// no store (server-level discovery, read-class).
	// An unknown method is UNIMPLEMENTED regardless of auth — you cannot
	// authorize an op that does not exist, and the gate would otherwise leak a
	// credential demand for a non-existent method. Gate KNOWN ops only.
	if op != '' {
		mut gate_mount := ''
		if m := pb_decode_store_request(call.message) {
			gate_mount = svc_resolve_sname(m.store, ctx.mounts)
		}
		mut nc := grpc_nonces()
		if e := grpc_call_gate(authz, path, call.message, op, gate_mount, ctx, mut nc) {
			return e
		}
	}
	match op {
		...
		'query' {
			...
			// the `path` query-param so the shared query route runs it. Each
			// [result doc= source= MATCH] L97 flat-relation tuple in the
			// [query-result …] body becomes one QueryRow DATA frame (the serialized
			// tuple, verbatim — cross-transport parity by construction).
			m := pb_decode_query_request(call.message) or { return grpc_invalid('query: bad request') }
			if m.query.len > 0 && m.comp.len > 0 {
				return grpc_invalid('query: query/comp are mutually exclusive')
			}
			params := if m.comp.len > 0 {
			...
			resp := svc_handle_request(grpc_synth_req('query', m.store, '', params, authz,
				tp), mut state, ctx)
			if e := grpc_resp_error(resp) {
				return e
			}
		...
		'put-blob' {
			// The F1' OPAQUE write (S6.5): the raw bytes ride the Put message
			// shape VERBATIM (encoding "raw" — no canonicalization, identity =
			// hash of the bytes as given); the reply's hash field carries the
			// blob KEY. Same profile-pipeline core as the XSP listener's arm.
			m := pb_decode_put_request(call.message) or {
				return grpc_invalid('put-blob: bad request')
			}
			resp := svc_handle_request(grpc_synth_req('put-blob', m.store, m.body.bytestr(),
				map[string]string{}, authz, tp), mut state, ctx)
			if e := grpc_resp_error(resp) {
				return e
			}
			body := svc_response_body(resp)
			return grpc_unary(pb_encode_put_response(GrpcPutResponse{
				hash:   grpc_attr_of(body, 'key')
				stored: grpc_attr_of(body, 'stored') == 'true'
			}))
		}
		'get-blob' {
			// The F1' OPAQUE read: raw bytes back, byte-exact (encoding "raw").
			// Absence maps NOT_FOUND with the cx-err trailer — the client
			// surfaces the blob surface's CXER1121 contract (error identity
			// across transports, §3).
			m := pb_decode_get_request(call.message) or {
				return grpc_invalid('get-blob: bad request')
			}
			resp := svc_handle_request(grpc_synth_req('get-blob', m.store, '', {
				'hash': m.hash
			}, authz, tp), mut state, ctx)
			if e := grpc_resp_error(resp) {
				return e
		...
		'aliases-set' {
			...
			// shape drives all seven — synthesize the pipeline="profile" request and
			// route it through the shared svc pipeline into svc_profile_data_op (W6;
			// same op semantics + wire bodies as the transitional CSRP route — exact
			// cross-transport parity, spec §6.1). The hex object bytes ride the cxd
			// body unchanged.
		...
		'status', 'gc', 'mounts', 'config-reload' {
			...
			// the blob pair + the object wire + the admin plane are all wired above —
			// the full op surface.)
			return GrpcReply{
...
fn grpc_op_from_path(path string) string {
	...
	return match method {
		...
		// the F1' OPAQUE blob pair (S6.5 — G13 edge parity; spec §2).
		'PutBlob' { 'put-blob' }
		'GetBlob' { 'get-blob' }
...
fn grpc_synth_req_m(op string, store string, body string, qparams map[string]string, authz string, traceparent string, method string) cx.Element {
	...
	// I5 stream 4 W6: pipeline="profile" routes the data op through
	// svc_profile_data_op (store_profile_ops.v) instead of store_csrp_route —
	// the gRPC adapter's data-op core survives the W7 CSRP-router deletion.
	return cx.Element{
		name:  'request'
		attrs: [
			cx.Attribute{
...
// grpc_query_rows parses a [query-result [result doc= source= MATCH] …] wire
// body into one serialized [result …] tuple per MATCH (the L97 flat relation) —
// each becomes a QueryRow DATA frame. The tuple is rendered canonically so the
// gRPC row carries exactly the executor's tuple (cross-transport parity by
// construction).
fn grpc_query_rows(body string) [][]u8 {
	...
	for el in doc.elements {
		if el is cx.Element && el.name == 'query-result' {
			for r in el.items {
				// path= rows are the L97 [result …] tuples; comp= rows ride
				// the [item …] envelope (L99 — lossless for scalar rows).
				if r is cx.Element && r.name in ['result', 'item'] {
...
// grpc_list_hashes extracts the TAGGED addresses (I1 stream 19) from a
// [list-result [hashes [hash "h"] …]] wire body.
fn grpc_list_hashes(body string) []string {
	...
	for {
		...
		h := body[hs..end]
		if _, _ := cx.cx_parse_tagged_address(h) {
			out << h
		}
		from = end + 1
	}
