...
mut:
	...
	bearer string
	// S3 (G1a): the client's XSP identity for the gRPC edge — did:key + its
	// 32-byte Ed25519 seed (resolved from open-opts xsp-did/xsp-seed-env).
	// Empty seed = anonymous (floor: data ops open, admin denied). Used to
	// sign each gRPC call's CxCall credential (grpc_call_auth_header).
	did  string
	seed []u8
	...
	// cx-store(+xsp):// — the XSP store-profile client session (I5 stream 4
	// W6): one persistent, tenant-bound attach per open handle, created at
	// parse; identity (open-opts xsp-did/xsp-seed-env) is threaded in by
	// store_open_impl before the first op dials.
	xsp &XspClientSession = unsafe { nil }
}
...
fn store_remote_parse(url string) (&RemoteBackend, cx.Node, bool) {
	...
	match scheme {
		...
		'cx-store', 'cx-store+xsp' {
			// cx-store://host:port/store-name/ — THE store wire (store.md §2):
			// the XSP store profile over TLS. cx-store+xsp:// is the cleartext
			// sibling (loopback/dev — the +http of the profile era). No URL
			// userinfo: identity rides open-opts (xsp-did + xsp-seed-env, the
			// seed always via env), never a URL credential. The port is
			// REQUIRED — the profile listener has no registered default.
			rest := url[scheme.len + 3..] // strip '<scheme>://'
			mut authority := rest
			mut storename := ''
			if sl := rest.index('/') {
			...
			if authority.contains('@') {
				return unsafe { nil }, mk_err('cx-err:CXER1100', 'E_STORE_UNRESOLVED_BACKEND: ${scheme}:// carries no URL userinfo — client identity rides open-opts (xsp-did + xsp-seed-env): ${store_url_redact_userinfo(url)}'), false
			}
			mut host := authority
			mut port := 0
			if colon := authority.last_index(':') {
				host = authority[..colon]
				port = authority[colon + 1..].int()
			}
			if host == '' || port <= 0 {
				return unsafe { nil }, mk_err('cx-err:CXER1100', 'E_STORE_UNRESOLVED_BACKEND: ${scheme} URL needs host:port (the [xsp] listener address): ${url}'), false
			}
			tls := scheme == 'cx-store'
			rb := &RemoteBackend{
				scheme: scheme
				...
				port:   port
				dir:    storename
				xsp:    &XspClientSession{
					mu:       sync.new_mutex()
					tls:      tls
					host:     host
					port:     port
					tenant:   storename
					endpoint: '${if tls { 'tls' } else { 'tcp' }}://${host}:${port}'
				}
			}
			return rb, store_null(), true
		}
		'cx-store+http', 'cx-store+https' {
			// RETIRED (stream-4 S3): the CSRP data plane is gone. The store
			// service wire is the XSP store profile — cx-store:// (TLS) /
			// cx-store+xsp:// (cleartext dev) — or the gRPC edge
			// cx-store+grpc(s)://. Cutover-first: no dual-accept window.
			return unsafe { nil }, mk_err('cx-err:CXER1100',
				'E_STORE_UNRESOLVED_BACKEND: the ${scheme} scheme is retired with the CSRP data plane — use cx-store:// (XSP store profile) or cx-store+grpc(s):// for ${url}'), false
		}
...
// store_remote_blob_put / store_remote_blob_get — the F1' OPAQUE pair for
// the SERVICE wires (S6). The born-remote object backends (s3/http/ftp/sftp)
// never route here: their blob records persist through their own manifest
// sinks (the store_append path the F1' substrate work covered). The gRPC
// edge carries the pair since S6.5 (G13 parity; cxstore-grpc.md §2) —
// PutBlob/GetBlob, raw bytes byte-exact, CXER1121 absence identity.
fn store_remote_blob_put(rb &RemoteBackend, raw string) cx.Node {
	match rb.scheme {
		'cx-store', 'cx-store+xsp' {
			return xsp_client_put_blob(rb, raw)
		}
		'cx-store+grpc', 'cx-store+grpcs' {
			return grpc_client_put_blob(rb, raw)
		}
		else {
			return mk_err('cx-err:CXER1100', 'E_STORE_UNRESOLVED_BACKEND: blob routing for ${rb.scheme}')
		}
	}
}

fn store_remote_blob_get(rb &RemoteBackend, key string) cx.Node {
	match rb.scheme {
		'cx-store', 'cx-store+xsp' {
			return xsp_client_get_blob(rb, key)
		}
		'cx-store+grpc', 'cx-store+grpcs' {
			return grpc_client_get_blob(rb, key)
		}
		else {
			return mk_err('cx-err:CXER1100', 'E_STORE_UNRESOLVED_BACKEND: blob routing for ${rb.scheme}')
		}
...
fn store_remote_put(rb &RemoteBackend, hash string, text string) cx.Node {
	match rb.scheme {
		...
		'cx-store', 'cx-store+xsp' {
			return xsp_client_put(rb, hash, text)
		}
		'cx-store+grpc', 'cx-store+grpcs' {
...
fn store_remote_delete(rb &RemoteBackend, hash string) (bool, cx.Node, bool) {
	match rb.scheme {
		...
		'cx-store', 'cx-store+xsp' {
			return xsp_client_delete(rb, hash)
		}
		'cx-store+grpc', 'cx-store+grpcs' {
...
fn store_remote_query(rb &RemoteBackend, cxpath string) cx.Node {
	match rb.scheme {
		'cx-store', 'cx-store+xsp' {
			return xsp_client_query(rb, cxpath)
		}
		'cx-store+grpc', 'cx-store+grpcs' {
...
// service_scheme reports whether a remote scheme is the store SERVICE tier
// (the XSP store profile or the gRPC edge) — the tier with server-side ops
// beyond the byte-source surface (iter/modify/query pushdown, the object
// wire, and the #248 admin plane). The transitional CSRP HTTP tokens are
// RETIRED (stream-4 S3).
fn service_scheme(s string) bool {
	return s in ['cx-store', 'cx-store+xsp', 'cx-store+grpc', 'cx-store+grpcs']
}
...
fn store_remote_modify(rb &RemoteBackend, hash string, action_text string) cx.Node {
	match rb.scheme {
		'cx-store', 'cx-store+xsp' {
			return xsp_client_modify(rb, hash, action_text)
		}
		'cx-store+grpc', 'cx-store+grpcs' {
...
// store_is_doc_hash — a stored doc object is named by its SELF-DESCRIBING
// tagged address (I1 stream 19: `<algo>:<lowercase-hex>`, registry-
// validated); the list filter excludes any non-doc objects sharing the
// prefix. Bare hex is not an address any more.
fn store_is_doc_hash(s string) bool {
	_, _ := cx.cx_parse_tagged_address(s) or { return false }
	return true
}
