...
fn fabric_open_remote(url string, opts cx.Node) cx.Node {
	...
	// §4.4a: semantic tokens are OFFERED inside the signed transcript — the
	// reference client speaks the full fabric set (xsp.md §5.0 surface 1).
	mut hello_items := [
...
fn fab_remote_publish(mut fr FabricRemote, args []cx.Node) cx.Node {
	...
	att := if args.len > 3 { args[3] } else { cx.Node(cx.Element{ name: code.map_marker_name }) }
