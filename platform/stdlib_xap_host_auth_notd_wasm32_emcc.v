...
fn xap_host_attach(mut h XapHost, raw_body string, hdrs XspReqHdrs) WireResp {
	return xap_auth_attach(mut h.auth, raw_body, hdrs)
}

// xap_auth_attach is the handshake endpoint decoupled from XapHost (#839
// R7.2): [$xap:serve {auth: [host-auth …]}] runs the SAME M1–M4 attach the
// deployment host runs, over the same auth object.
fn xap_auth_attach(mut auth XapHostAuth, raw_body string, hdrs XspReqHdrs) WireResp {
...
fn xap_host_attach_m1(mut auth XapHostAuth, m1 cx.Element) WireResp {
	...
	opts := cx.Node(cx.Element{
		...
		items: [
			...
			// §4.4a: the host's semantic token set, offered inside the signed
			// transcript. The xap host serves the XAP profile; its session
			// features negotiate per-surface below the profile.
			session_kv('offer-profiles', crypto_string_node('xap')),
			session_kv('offer-features', crypto_string_node('')),
		]
	})
...
fn xap_host_auth_admit(mut h XapHost, hdrs XspReqHdrs, raw_body string) XapAuthAdmit {
	return xap_auth_admit(mut h.auth, hdrs, raw_body)
}

// xap_auth_admit is the §4.8 rule-2 possession-proof check decoupled from
// XapHost (#839 R7.2) — serve and host verify identically.
fn xap_auth_admit(mut auth XapHostAuth, hdrs XspReqHdrs, raw_body string) XapAuthAdmit {
