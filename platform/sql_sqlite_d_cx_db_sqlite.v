...
fn new_sqlite_conn(url string) !SqlConn {
	mut path := url
	// Both DSN spellings (#520): authority form `sqlite:///file.db` and the
	// opaque form `sqlite::memory:` / `sqlite:file.db`. sqlite's own
	// `:memory:` target must survive the strip verbatim.
