...
fn (mut t RotStubTransport) remove(key string) (int, bool) {
	t.blobs.delete(key)
	return 204, true
}
...
// ── #779: rotation vs the background fold worker ─────────────────────────────
//
// The compacted rewrite (write_compacted_keyed: b.segs = [] + gen bump) must
// hold the store op-lock. Pre-fix, `store-rotate-kek` dispatched through
// store_stdlib_builtin_inner — the `cx store-rotate-kek` CLI wrapper
// (svc_rotate_kek) and daemon path — ran the whole walk UNLOCKED: its own
// store_persist kicks the background fold worker, and the unlocked segment-set
// reset could land inside the worker's locked fold_commit between the position
// scan and the b.segs[lo_pos] assignment (observed: V panic `array.set: index
// out of range (0,0)` on the store_cxpack_fold_worker thread, s1_w5_gate.log
// lane [70/84]). The interleave window is µs-narrow, so this drives the exact
// pre-fix path in a tight loop as a regression guard: every round builds a
// fold-due segment tower (each persist flushes one segment and kicks the
// worker), then rotates immediately through the inner dispatch. Post-fix the
// walk holds the (reentrant) op-lock, closing the window by construction:
// rotation and fold_commit serialize, every rotation succeeds, every doc
// stays readable.
fn test_rotation_cxpack_vs_background_fold_worker_779() {
	dir := os.join_path(os.temp_dir(), 'cx_rot_race_${os.getpid()}')
	os.rmdir_all(dir) or {}
	defer {
		os.rmdir_all(dir) or {}
		rot_unsetenv_ab()
	...
	mut hashes := []string{}
	mut texts := []string{}
	keys := ['tenant_b', 'tenant_a'] // alternate so both env keys always resolve
	for round in 0 .. 24 {
		// A tower of same-sized segments: each persist flushes one segment and
		// (two or more live segments within 2x of each other) kicks the fold
		// worker, so a fold is in flight when the rotation below starts.
		for i in 0 .. 3 {
			c, hh := rot_canon_hash('[doc [round ${round}] [i ${i}] [pad "0123456789abcdef"]]')
			store_put_canonical(mut ms, hh, c) or { panic(err) }
			store_persist(mut ms) or { panic(err) }
			hashes << hh
			texts << c
		}
		rep := rot_call('store-rotate-kek', [h, store_str(keys[round % 2])])
		assert rot_err_code(rep) == '', 'rotate round ${round}: ${rot_err_code(rep)} ${rot_attr(rep,
			'message')}'
		assert rot_attr(rep, 'objects').int() > 0, 'round ${round} walked no objects'
	}

	// Every doc written across every round survives the rotation storm.
	for i, hh in hashes {
		g := store_doc_text(ms, hh) or { panic('doc ${i} after rotation storm: ${err.msg()}') }
		assert g == texts[i]
	}
	rot_call('store-close', [h])
}
