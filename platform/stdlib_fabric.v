...
mut:
	...
	active  bool
	// declared_consistency is the stream-7 per-subscription floor (L123):
	// the `consistency` subscribe-opts tokens, validated ONCE at subscribe
	// against fab_sub_guarantee_advert. Empty = undeclared.
...
fn fab_sub_elem(f &FabricState, s &FabricSub) cx.Node {
	...
	// the delivery §4 contract attrs (additive — RULED: U1.2a; #762 V4):
	// the durable plane is journal-backed (retention=full), pull-flow;
	// sharing follows the group axis.
	attrs << bus_attr('rung', ':complete-ordered')
	attrs << bus_attr('sharing', if s.group == '' { 'independent' } else { 'group' })
	attrs << bus_attr('flow', 'pull')
	attrs << bus_attr('retention', 'full')
	attrs << bus_attr('on-close', 'fabric-unsubscribe')
	// Stream 7 (L123): the declared floor is inspectable on the sub element.
	if s.declared_consistency.len > 0 {
		mut toks := []string{}
		for t in s.declared_consistency {
			toks << ':${t}'
		}
		attrs << bus_attr('consistency', toks.join(' '))
	}
...
// fab_sub_guarantee_advert is the guarantee set a fabric SUBSCRIPTION
// advertises (stream 7 L123): the durable plane reads a journal stream —
// committed prefix, position pins (from=), forward-only cursor, gapless
// under the retention guard. `:at-least-once` is the GROUP plane's
// delivery class only (redelivery-until-ack, §9.1/§19.3); an observe or
// ungrouped subscription is a cursor replay with no redelivery contract
// and refuses it loudly (the plane discriminator).
fn fab_sub_guarantee_advert(group string) []string {
	mut adv := ['prefix-consistent', 'at-seq-pinned', 'monotonic-reads', 'gapless']
	if group != '' {
		adv << 'at-least-once'
	}
	return adv
...
fn fab_subscribe_common(args []cx.Node, observe bool) cx.Node {
	...
	// Stream 7 (L122/L123): the per-subscription consistency floor —
	// validated ONCE here against the plane's advertised set (group subs
	// advertise :at-least-once; observe/ungrouped refuse it — the plane
	// discriminator). Declared :gapless makes an explicit from= below the
	// stream's retained floor refuse (CXER4991) instead of the silent
	// clamp-to-seam replay (F2's fabric half; the XSP observe-resume
	// server checks ride this same floor).
	surface := if observe { 'fabric:observe' } else { 'fabric:subscribe' }
	optsn := if args.len > 3 { args[3] } else { cx.Node(cx.Element{ name: 'map' }) }
	cdecl, cerr, cok := cst_read_declared(optsn, surface)
	if !cok {
		return cerr
	}
	if cdecl.len > 0 {
		if e := cst_check_floor(cdecl, surface, fab_sub_guarantee_advert(group)) {
			return e
		}
		if 'gapless' in cdecl && has_from {
			base := i64(jrn_stream_base(j, stream))
			if base > 0 && from <= base {
				return cst_pin_refusal(surface, 'gapless', int(from), int(base),
					'subscribe from above the retained floor, or fold-from over the covering snapshot')
			}
		}
	}
	f.next_sub++
	sub := &FabricSub{
		...
		dlq:            eff_dlq
		declared_consistency: cdecl
	}
	f.subs[sub.id] = sub
	return fab_sub_elem(f, sub)
}

fn fabric_subscribe(args []cx.Node, mut env MatchEnv) cx.Node {
	return fab_stamp_sub(fab_subscribe_common(args, false), mut env)
}

fn fabric_observe(args []cx.Node, mut env MatchEnv) cx.Node {
	return fab_stamp_sub(fab_subscribe_common(args, true), mut env)
}

// fab_stamp_sub makes a [fabric-sub] closeable: the close cancels exactly
// that subscription (active=false), idempotently, via the registry.
fn fab_stamp_sub(el cx.Node, mut env MatchEnv) cx.Node {
	if el is cx.Element {
		if el.name == 'fabric-sub' {
			fid := el.attr('handle').int()
			sid := el.attr('id').int()
			return bus_stamp_closeable(el, 'fabric-unsubscribe', fn [fid, sid] () ! {
				f := fabric_lookup(fid) or { return }
				mut sub := f.subs[sid] or { return }
				sub.active = false
			}, mut env)
		}
	}
...
// ── the fabric-sub consumption arms (Ring2SubOps; #762 V2) ────────────────
//
// [?receive]/[?select] accept a [fabric-sub] directly (delivery §4;
// RULED: U1.2a/U1.5a): the receive is fabric_receive's cursor-advancing
// scan — unbatched answers ONE event entry (or fabric's own empty-answer
// posture, the empty node-set, on a quiet tail); the batch form collects
// up to max within deadline. Readiness is head-past-cursor on the sub's
// stream (a pattern-filtered sub may fire ready on non-matching traffic
// and receive the honest empty answer — the filter is applied at consume,
// fabric §19.1; recorded latitude).

fn fab_sub_contract_receive(sub cx.Node, max int, deadline_ms i64, mut env MatchEnv) cx.Node {
	// ONE receive = ONE scan turn — the retired module verb's exact
	// semantics (fabric.md §19.1: "identical semantics", the U1.12a
	// cutover): the embedded scan returns what is committed now (never
	// blocks); the remote wire blocks server-side up to deadline. No
	// local re-poll — a windowed observe must push exactly its window.
	want := if max > 0 { max } else { 1 }
	mut mitems := [
		cx.Node(cx.Element{
		name:  'max'
		items: [cx.Node(bus_int(i64(want)))]
	}),
	]
	if deadline_ms >= 0 {
		mitems << cx.Node(cx.Element{
			name:  'deadline'
			items: [cx.Node(bus_int(deadline_ms))]
		})
	}
	mopts := cx.Element{
		name:  '__cx_map__'
	...
	mut r := cx.Node(bus_null())
	if rr := fabric_remote_builtin('fabric-receive', [sub, cx.Node(mopts)]) {
		// the pattern was applied server-side — the same routing the
		// retired module verb used.
		r = rr
	} else {
		r = fabric_receive([sub, cx.Node(mopts)], mut env) or { bus_null() }
	}
	if is_err_value(r) {
		return r
	}
...
fn fab_sub_contract_ready(sub cx.Node, mut env MatchEnv) bool {
	_ = env
	f, s, _, ok := fab_get_sub(sub)
	if !ok {
		return true // the fault surfaces loud on the receive
	}
	mut head := i64(0)
	if hn := journal_stdlib_builtin('journal-head', [f.journal_elem,
		cx.Node(bus_str(s.stream))]) {
	...
	// s.cursor is the NEXT seq the subscription scans (fabric §19.3) — an
	// unscanned committed entry exists when head reaches it.
	return head >= i64(s.cursor)
}
...
fn fabric_stdlib_builtin(name string, args []cx.Node) ?cx.Node {
	...
	match name {
		...
		// 'fabric-subscribe' / 'fabric-observe' live on the env-aware chain
		// (#762 V4): the returned subscription is closeable (SAP §5.1),
		// and the close stamp needs the program's closeable registry.
		'fabric-ack' {
...
fn fabric_stdlib_builtin_env(name string, args []cx.Node, mut env MatchEnv) ?cx.Node {
	match name {
		'fabric-subscribe' {
			...
			return fabric_subscribe(args, mut env)
		}
		'fabric-observe' {
			if r := fabric_remote_builtin(name, args) {
				return r
			}
			return fabric_observe(args, mut env)
		}
		// 'fabric-receive' is RETIRED (RULED: U1.12a — the cutover to the
		// core [?receive from= max= deadline=] spelling; fabric.md §15/§19.1).
		// The fabric_receive scan lives on as the [fabric-sub] contract
		// arm's engine (fab_sub_contract_receive).
		'fabric-request' {
...
// The cx-fabric module surface (stdlib/fabric.cx) is embedded as
// stdlib_src_fabric in Ring-1 stdlib_bundle.v with the other bundle
// sources (relocated at I3 seam H) and registered as its OWN package
// there (the cx-xap shape).
