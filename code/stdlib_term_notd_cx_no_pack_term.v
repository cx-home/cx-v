...
fn term_stdlib_builtin(name string, args []cx.Node) ?cx.Node {
	...
	match name {
		...
		'term-select' {
			...
			for si, s in sources {
				// DP4 (h) — a `[file handle=N …]` process-stream handle (a
				// child pipe or a pty master) resolves to the raw fd it wraps
				// via io_handle_raw_fd: a shell selects on its child's pty
				// exactly like on a socket. Per term.md §3.6, a `[file …]`
				// handle backed by a buffered os-level file is not a
				// selectable source and is SKIPPED — the one spec-stated
				// exception to #852's refusal below.
				if s is cx.Element {
					if s.name == 'file' {
						if fd := io_handle_raw_fd(s) {
							fds << fd
							src_at << si
						}
						continue
				...
				// #852 — an unresolvable source REFUSES rather than being
				// skipped. The old loop dropped it silently, which is how a
				// source that could never fire looked exactly like a source
				// that simply had no data.
				fd := term_source_fd(s) or {
					return mk_err(term_err_io, 'E_TERM_IO: select source ${si}: ${err.msg()}')
				}
				fds << fd
				src_at << si
			}
...
// term_source_fd resolves ONE `[?term:select]` source handle to the OS
// descriptor poll(2) can wait on.
//
// #852 — THE `fd=` VALUE IS NOT A DESCRIPTOR ON ALMOST EVERY HANDLE. This
// function used to return it verbatim. But `net_register` hands out
// `next_id++`, so `[socket]`, `[exchange]`, `[http-server]` and
// `[sse-source]` all carry a REGISTRY ID in `fd=` — the first two
// connections in a process get 1 and 2, which are stdout and stderr. So
// `select` dutifully polled stdout, the source arm never fired, and nothing
// errored. #852 was filed against SSE because that is where a consumer
// noticed; every source kind was affected.
//
// The exception is `[std-stream fd=0|1|2]`, which never went through the net
// registry and does carry a real descriptor. So the resolution is by ELEMENT
// NAME, which is unambiguous, rather than by guessing from the number:
//
//   std-stream                                  -> the value IS a descriptor
//   socket / listener / exchange / http-server /
//   sse-source                                  -> registry id, resolved
//   anything else                               -> refuse
//
// `[file handle=N …]` process-stream handles never reach this function: the
// select loop resolves them via io_handle_raw_fd first (DP4 (h)), and a
// buffered-file-backed one is skipped per term.md §3.6 — the one spec-stated
// exception to the refusal rule.
//
// term.md §3.6's "anything carrying an `fd=` attribute" stays true — sources
// are still fd-bearing handles and the surface is unchanged. What changed is
// that this function asks the runtime to resolve the id instead of handing it
// to poll(2) raw, which is also what keeps term decoupled from each source's
// wire framing: it never learns what an SSE frame or an HTTP exchange is.
//
// The alternative — putting the real descriptor in `fd=` — was rejected. It
// would leak a descriptor the runtime owns into the CX surface, invite user
// code to act on it behind the runtime's back, and leave one attribute name
// meaning an id on some handles and a descriptor on others.
fn term_source_fd(n cx.Node) !int {
	if n !is cx.Element {
		return error('not a handle element')
	}
	el := n as cx.Element
	raw := term_handle_fd_attr(n) or {
		return error('[${el.name}] carries no `fd=` attribute')
	}
	if el.name == 'std-stream' {
		// A standard stream's fd IS the descriptor — it predates the registry.
		return raw
	}
	if el.name in ['socket', 'listener', 'exchange', 'http-server', 'sse-source'] {
		$if !cx_no_pack_http_client ? {
			if resolved := net_pollable_fd_id(raw) {
				return resolved
			}
			return error('[${el.name} fd=${raw}] is a closed or unbacked handle — nothing to poll')
		} $else {
			return error('[${el.name}] needs the http-client pack to resolve its descriptor')
		}
	}
	return error('[${el.name}] is not a pollable source handle')
}

// term_handle_fd_attr reads the raw `fd=` integer off a handle element. What
// that integer MEANS depends on the element — see term_source_fd.
fn term_handle_fd_attr(n cx.Node) ?int {
	if n is cx.Element {
		for a in n.attrs {
			if a.name == 'fd' {
				v := a.value
