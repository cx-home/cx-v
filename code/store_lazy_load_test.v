...
fn test_lazy_open_pages_objects_on_first_touch() {
	...
	assert csrp_scalar(ga) == keys[4], 'alias resolves after a lazy open: ${render_canonical(ga)}'
	...
	assert csrp_scalar(g).contains('payload 1'), 'paged read content: ${csrp_scalar(g)}'
	...
	assert csrp_scalar(fr).contains('payload 6'), 'write-after-lazy-read round trip'
	...
	for i, k in keys {
		...
		assert csrp_scalar(r).contains('payload ${i + 1}'), 'doc ${i + 1} survived: ${csrp_scalar(r)}'
	}
	r6 := store_stdlib_builtin_inner('store-get-doc-text', [h2, store_str(k6)]) or { panic('reread 6') }
	assert csrp_scalar(r6).contains('payload 6'), 'the post-lazy write survived the reopen'
}
