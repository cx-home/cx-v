...
// `--- out_err`.
//
// NOTE on `parse-args`: the public `parse-args(spec)` parses the live
// process argv (`os.args`). To keep the parser deterministically
...
// `os.args`; with two it parses the supplied sequence. The pure
...
// (the spec) it parses the live process argv; with a second arg (an
// explicit argv sequence) it parses that — used by the deterministic
