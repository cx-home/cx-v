...
pub fn io_arg_str(n cx.Node) ?string {
...
pub fn io_arg_int(n cx.Node) ?i64 {
	if n is cx.ScalarNode {
		v := n.value
		match v {
			i64 { return v }
...
// io_handle_raw_fd resolves a `[file handle=N …]` element to the raw OS fd it
// wraps, when it wraps one (a child_fd-backed handle: a spawned child's
// stdin/stdout/stderr, or a pty master). Returns none for os.File-backed
// handles — a buffered file is not a selectable source. This is DP4 (h)'s
// V-side resolver: it lets `term:select` treat a process-stream handle as an
// fd-bearing source without the process pack growing a second handle shape.
pub fn io_handle_raw_fd(n cx.Node) ?int {
	if n is cx.Element && n.name == 'file' {
		for a in n.attrs {
			if a.name == 'handle' {
				id := int(cx.scalar_value_str_public(a.value).int())
				h := io_lookup(id) or { return none }
				if h.is_open && h.child_fd >= 0 {
					return h.child_fd
				}
				return none
			}
...
// io_read_caps / io_write_caps (the §7 read-path / write-path cap lists the
// dispatch below fences on) live in effect_alignment.v — I4: profile-invariant
// purity data, outside this `-d cx_no_pack_io`-gated file.
...
fn io_stdlib_builtin(name string, args []cx.Node) ?cx.Node {
	...
	// The watch verbs live in stdlib_iowatch.v (+ platform backends) —
	// RING 2 since the I3 split. They dispatch through the I3 registry
	// (probed before this pack) and carry their read-path cap gate with
	// them (iowatch_ring2_builtin); watch-close — like close — needs none.
	...
	match name {
		...
		'io-temp-dir' {
			prefix := io_arg_str(args[0]) or { return none }
			base := os.temp_dir()
			// The contract is a FRESH, EMPTY directory every call. The token
			// alone is only process-unique: pids recycle and nothing cleans
			// the system temp dir, so a later generation could land on a
			// leftover directory and inherit its contents. mkdir (unlike
			// mkdir_all) fails when the name is taken, so retrying with a
			// fresh token is collision-proof by construction — no randomness
			// in the visible name shape needed.
			mut path := ''
			for _ in 0 .. io_temp_attempts {
				cand := os.join_path(base, '${prefix}${io_temp_token()}')
				if os.exists(cand) {
					continue
				}
				os.mkdir(cand) or {
					if os.exists(cand) {
						// Lost to a leftover (or a concurrent creator):
						// the name is taken, so draw the next token.
						continue
					}
					return io_os_err('temp-dir', cand, err.msg())
				}
				path = cand
				break
			}
			if path == '' {
				return io_os_err('temp-dir', os.join_path(base, prefix),
					'no free name after ${io_temp_attempts} attempts')
			}
			return io_str(path)
		}
		'io-system-temp-dir' {
...
// drawn from a monotonic counter. Uniqueness holds WITHIN a process only
// — pids recycle across generations, so callers that create a directory
// must retry on a taken name (see 'io-temp-dir'). 'io-temp-file' opens
// through os.create, which truncates, so a reused name never hands back
// a prior generation's contents.
__global g_io_temp_counter int

// io_temp_attempts bounds the fresh-name search so a pathological temp
// dir cannot spin forever; the counter advances every attempt, so the
// bound is only ever reached when the whole run of names is taken.
const io_temp_attempts = 128
