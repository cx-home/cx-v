...
//
// N spawned threads drive puts through store_csrp_route — the exact daemon
// dispatch path (svc_dispatch_data_op → store_csrp_route) — against ONE opened
// store handle, the daemon worker-pool shape. store_csrp_route serializes on the
...
fn conc_put(local cx.Node, i int) bool {
	...
	resp := store_csrp_route(grpc_synth_req('put', '', c, map[string]string{}, '', ''),
		local)
...
fn conc_list_count(local cx.Node) int {
	resp := store_csrp_route(grpc_synth_req('list', '', '', map[string]string{}, '', ''),
		local)
...
fn conc_stress(url string, tag string) {
	...
	if !url.starts_with('mem://') {
		...
		for i in 0 .. conc_writers {
			...
			gr := store_csrp_route(grpc_synth_req('get', '', '', {
				'hash': h
			}, '', ''), reopened)
...
// conc_resp_status extracts the HTTP status from a csrp_resp node.
...
fn conc_refs_set_cas(local cx.Node, key string, root string) int {
	resp := store_csrp_route(grpc_synth_req('refs-set', '',
...
fn test_racing_conditional_refs_set_exactly_one_wins() {
	...
	resp := store_csrp_route(grpc_synth_req('refs', '', '[refs [k key="${key}"]]',
		map[string]string{}, '', ''), local)
	body := svc_response_body(resp)
	cur := if body.contains(ra) { ra } else { rb }
	rc := 'cc'.repeat(32)
	retry := store_csrp_route(grpc_synth_req('refs-set', '',
	...
	rd := 'dd'.repeat(32)
	stale := store_csrp_route(grpc_synth_req('refs-set', '',
