...
fn sch_persist_intent(t &SchedTimer) {
	...
	// I3: durable-timer persistence reaches the Ring-2 journal pack BY
	// NAME through the registry — sched (Ring 1) holds no direct journal
	// reference. Unregistered (a profile without the journal pack) ⇒ the
	// probe returns none and the existing or-arm degrades persistence
	// gracefully, which is the correct profile behavior.
	ring2_stdlib_builtin('journal-append', [t.journal, intent, attribution]) or { return }
}
...
fn sch_persist_closed(t &SchedTimer, status string) {
	...
	ring2_stdlib_builtin('journal-append', [t.journal, closed, attribution]) or { return }
}
		...
		if t.ev is cx.Element {
			...
			if nm != '' {
				if mut ch := env.state.channel_get(nm) {
					...
					// A fan-out stream (sharing=independent, U1.11a) delivers
					// through its retention window — ch_enqueue on it would
					// strand the tick in the group queue where no subscriber
					// ever looks (found at supervise graduation, RULED: SUP-1
					// rider SUP-1e; the eval_select send-case makes the same
					// split).
					if ch.sharing == 'independent' {
						ch_fan_send(mut ch, tick)
					} else {
...
fn sch_restore(args []cx.Node, mut env MatchEnv) ?cx.Node {
	...
	entries_node := ring2_stdlib_builtin('journal-since', [journal, sch_int(1)]) or {
...
// stdlib_src_sched lives in stdlib_bundle.v (I4: source embeds are data,
