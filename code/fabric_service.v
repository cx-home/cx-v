...
fn fs_dispatch_locked(mut srv FabricServer, mut c FsConn, fe cx.Element) {
	...
	if ftype == 'ping' {
		b := fs_frame_bytes('pong', stream, bus_null()) or { return }
...
fn fs_dispatch_verb_locked(mut srv FabricServer, mut c FsConn, stream i64, req cx.Element) {
	match req.name {
		...
		'session' {
			// xsp.md §5.0 (#560): feature negotiation rides a post-attach
			// session query, NOT the XSP-AUTH M4 (whose shape belongs to the
			// graduated identity model). Old clients never ask; new clients
			// configure heartbeat cadence + flow control from the answer.
			fs_reply_locked(mut srv, c.id, stream, cx.Node(cx.Element{
				name:  'fabric-session'
				attrs: [
					bus_attr('features', 'heartbeat credit resume publish-batch rotate'),
	...
	for it in req.items {
		if it is cx.Element {
			if it.name == 'event' && it.items.len > 0 {
				...
				// host demotes claimed author=. Only expect-prev-seq — the
				...
				for kv in it.items {
					...
					match kve.name {
						...
						'expect-prev-seq' {
							...
							// contract — a batch has no one prev to expect.
							fs_error_locked(mut srv, c.id, stream, mk_err(fab_err_wire,
								'E_FABRIC_WIRE: expect-prev-seq does not compose with publish-batch (single-append optimistic concurrency) — use publish'))
							return
						}
						else {
							fs_error_locked(mut srv, c.id, stream, mk_err(fab_err_wire,
...
fn fs_emit_locked(mut srv FabricServer, mut c FsConn, stream i64, req cx.Element) {
	...
	for sid in targets {
		s := srv.subs[sid] or { continue }
		payload := cx.Node(cx.Element{
			name:  'channel-value'
			attrs: [bus_attr('channel', s.label)]
			items: [value]
		})
...
fn fs_rotate_run(mut srv FabricServer, job FsRotateJob) {
	...
	fs_retention_dispose(mut srv, job.tenant, old_url)
}
...
fn fs_retention_dispose(mut srv FabricServer, tenant string, sealed_url string) {
