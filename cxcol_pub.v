...
// Public CXCol/table surface for out-of-module consumers — today the
// optional `vcx/arrow/` module (libcx_arrow) and one conformance
// runner. Core libcx remains Arrow-free; this file only exposes
// existing CXCol primitives via stable names that those consumers can
// call without reaching into private symbols. (Renamed from
// `arrow_pub.v` at I2 — the file carries no Arrow dependency edge and
// Ring-0 vocabulary should not suggest one.)
//
...
// col_spec_to_ast_bin_nullable_pub is the §3.10.5-aware col-spec
// writer (stream 17 W3c — the Arrow import lane): columns flagged
// nullable emit the 0x80 wrapper code in the header.
pub fn col_spec_to_ast_bin_nullable_pub(cols []TableColumn, nullable []bool) []u8 {
	// The col-spec payload is the ast_bin `table`-element carrier
	// (col_spec_to_ast_bin) — nullability rides a `?` PREFIX on the
	// type-name attr value; the writer's header emission maps a
	// `?`-prefixed name to the 0x80 wrapper code.
	mut adjusted := []TableColumn{cap: cols.len}
	for i, col in cols {
		if i < nullable.len && nullable[i] {
			adjusted << TableColumn{
				name:      col.name
	...
	return col_spec_to_ast_bin(adjusted)
}
